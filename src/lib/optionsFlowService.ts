...
export class OptionsFlowService {
  ...
  async fetchLiveOptionsFlowUltraFast(
    ...
    if (ticker && (ticker.toLowerCase() === 'all' || ticker === 'ALL_EXCLUDE_ETF_MAG7')) {
      ...
      tickersToScan = ticker
        .split(',')
        .map((t) => (t && t.trim() ? t.trim().toUpperCase() : ''))
        .filter((t) => t)
    ...
    try {
      ...
      if (onProgress && filteredTrades.length > 0) {
        onProgress(
          filteredTrades,
          `[OK] Classification complete - sending ${filteredTrades.length} trades`
        )
      }
  ...
  async fetchLiveOptionsFlowStreaming(
    ...
    console.log(
      `[STREAM] STREAMING: Starting live options flow${ticker ? ` for ${ticker}` : ' market-wide scan'}`
    )

    const allTrades: ProcessedTrade[] = []
    const tickersToScan =
      ticker && ticker.toLowerCase() === 'all'
        ? this.getTop1000Symbols()
        : ticker
          ? [ticker.toUpperCase()]
          : []
    ...
    for (let batchIndex = 0; batchIndex < tickerBatches.length; batchIndex++) {
      ...
      onProgress?.(
        allTrades,
        `Processing batch ${batchIndex + 1}/${tickerBatches.length}: ${batch.join(', ')}`,
        {
          ...
          if (tickerTrades.length > 0) {
            ...
            onProgress?.(
              ...
              `Found ${tickerTrades.length} raw trades from ${currentTicker}`,
              {
                ...
                progress: (
                  ((batchIndex * batchSize + batch.indexOf(currentTicker)) / tickersToScan.length) *
                  100
                ).toFixed(1),
              }
            )
    ...
    onProgress?.(
      allTrades,
      `Classifying ${allTrades.length} trades for SWEEP/BLOCK/MINI detection...`
    )
    ...
    onProgress?.(
      classifiedTrades,
      `Scan complete: ${classifiedTrades.length} classified trades found`
    )
  ...
  async fetchLiveOptionsFlow(ticker?: string): Promise<ProcessedTrade[]> {
    ...
    if (ticker && ticker.toLowerCase() === 'all') {
      ...
      tickersToScan = this.getTop1000Symbols()
      console.log(
        `[ENRICH] FORCED 1000 STOCK SCAN: ${tickersToScan.length} symbols (NO UNIVERSAL TICKER)`
      )
    ...
    console.log(
      `[READY] LIVE TRADES SCANNING ${tickersToScan.length} tickers from today's market open...`
    )
    ...
    console.log(
      `[NOTE] Processing ${tickerBatches.length} batches of ${tickerBatchSize} stocks each with rate limiting...`
    )
    ...
    for (let batchIndex = 0; batchIndex < tickerBatches.length; batchIndex++) {
      const batch = tickerBatches[batchIndex]
      console.log(
        `[EXEC] Processing batch ${batchIndex + 1}/${tickerBatches.length}: ${batch.slice(0, 5).join(', ')}...`
      )
          ...
          try {
            ...
            if (isLive) {
              ...
              if (trades.length > 0) {
                ...
                console.log(
                  `[WARN] ${symbol}: No live trades yet today - this is normal early in trading`
                )
              }
            } else {
            ...
            if (
              ...
              console.warn(
                `[CONN] ${symbol}: Connection reset, retrying... (${retries} attempts left)`
              )
              if (retries > 0) {
      ...
      console.log(
        `[OK] Batch ${batchIndex + 1} complete: ${allTrades.length} total trades found so far`
      )
    ...
    console.log(
      `[READY] INDIVIDUAL TRADES COMPLETE: ${allTrades.length} total individual trades collected`
    )
    ...
    if (allTrades.length > 0) {
      ...
      return filtered.sort(
        (a: ProcessedTrade, b: ProcessedTrade) => b.total_premium - a.total_premium
      )
    }
  ...
  private filterAndClassifyTrades(
    ...
    if (
      targetTicker &&
      targetTicker.toLowerCase() !== 'all' &&
      targetTicker !== 'ALL_EXCLUDE_ETF_MAG7'
    ) {
      ...
      if (targetTicker.includes(',')) {
        ...
        console.log(
          `[FILTER] Filtering for multiple tickers: ${tickerList.join(', ')} - ${filtered.length} trades match`
        )
      } else {
    ...
    if (!alreadyClassified) {
      console.log(
        `[PROC] Starting multi-leg detection on ${filtered.length} unclassified trades...`
      )
  ...
  private detectSweeps(trades: ProcessedTrade[]): ProcessedTrade[] {
    ...
    exactTimeGroups.forEach((tradesInGroup, groupKey) => {
      ...
      if (exchanges.length >= 2) {
        ...
        console.log(
          `[SWEEP] SWEEP DETECTED: ${sweepTrade.ticker} $${sweepTrade.strike} ${sweepTrade.type.toUpperCase()}S - ${totalContracts} contracts, $${totalPremium.toLocaleString()} across ${exchanges.length} exchanges`
        )
  ...
  private detectMultiLegTrades(trades: ProcessedTrade[]): ProcessedTrade[] {
    ...
    for (const [groupKey, groupTrades] of exactTimeGroups) {
      ...
      if (isMultiLeg) {
        console.log(
          `[OK] MULTI-LEG CONFIRMED: ${groupTrades.length} legs for ${groupTrades[0].underlying_ticker}`
        )
        multiLegCount++
    ...
    console.log(
      `[OK] MULTI-LEG DETECTION COMPLETE: Found ${multiLegCount} multi-leg strategies from ${trades.length} individual trades`
    )
  ...
  private analyzeMultiLegPattern(trades: ProcessedTrade[]): boolean {
    ...
    if (!allLegsHave100Plus) {
      ...
      console.log(
        `   [ERROR] Rejected: Legs with <100 contracts: ${smallLegs.map((t) => `${t.ticker}:${t.trade_size}`).join(', ')}`
      )
      return false
    }
    ...
    if (!hasMultipleStrikes && !hasMultipleTypes && !hasMultipleExpirations) {
      console.log(
        `   [ERROR] Rejected: All same strike (${uniqueStrikes.size}), type (${uniqueTypes.size}), expiry (${uniqueExpirations.size})`
      )
      return false
    }

    const isMultiLeg =
      substantialPremium && (hasMultipleStrikes || hasMultipleTypes || hasMultipleExpirations)
  ...
  private async robustFetch(url: string, maxRetries: number = 3): Promise<Response> {
    ...
    return withCircuitBreaker('polygon', async () => {
      ...
      for (let attempt = 1; attempt <= maxRetries; attempt++) {
        try {
          ...
          const timeoutId = setTimeout(() => {
            console.warn(
              `[TIME] Aborting request after 3s timeout (attempt ${attempt}/${maxRetries})`
            )
            controller.abort()
          ...
          if (!response.ok) {
            ...
            console.error(
              `[ERROR] API Error: ${errorMsg} for URL: ${url.replace(this.polygonApiKey, 'API_KEY_HIDDEN')}`
            )

            // Handle specific error codes with progressive backoff
            if (response.status === 403) {
              console.error(
                `[ERROR] HTTP 403 Forbidden - Check API key permissions and rate limits`
              )
              ...
              console.error(
                `[ERROR] HTTP ${response.status} Server Error - Server is experiencing issues`
              )
              if (attempt < maxRetries) {
          ...
          if (error instanceof Error) {
            if (error.name === 'AbortError') {
              console.warn(
                `[TIME] Request timeout (attempt ${attempt}/${maxRetries}) - Network may be slow`
              )
            } else if (
              error.message.includes('Failed to fetch') ||
              ...
              console.warn(
                `[CONN] Connection error (attempt ${attempt}/${maxRetries}): ${error.message}`
              )
              ...
              console.warn(
                `[NET] Network connectivity issue (attempt ${attempt}/${maxRetries}): ${error.message}`
              )
            } else {
              console.warn(
                `[RETRY] Fetch attempt ${attempt}/${maxRetries} failed: ${error.message}`
              )
            }
          }
          ...
          if (attempt < maxRetries) {
            ...
            console.warn(
              `[WAIT] Retrying in ${(delay / 1000).toFixed(1)}s... (${maxRetries - attempt} attempts remaining)`
            )
            await new Promise((resolve) => setTimeout(resolve, delay))
          }
        }
      ...
      console.error(
        `[ERROR] All ${maxRetries} attempts failed for URL: ${url.replace(this.polygonApiKey, 'API_KEY_HIDDEN')}`
      )
  ...
  async fetchLiveStreamingTradesRobust(ticker: string): Promise<ProcessedTrade[]> {
    ...
    try {
      ...
      console.log(
        `[FILTER] ${ticker}: ${allContracts.length - validContracts.length} deep ITM contracts filtered out`
      )
      ...
      console.log(
        `[SCAN] ${ticker}: Scanning trades for ALL ${validContracts.length} valid contracts (all expirations)...`
      )
      ...
      console.log(
        `[MODE] MODE: ${validContracts.length} contracts -> ${contractBatches.length} batches (${BATCH_SIZE} each)`
      )
      console.log(
        `[OPT] API calls reduced from ${validContracts.length} to ${contractBatches.length} (${((1 - contractBatches.length / validContracts.length) * 100).toFixed(1)}% reduction)`
      )
      ...
      for (let batchIndex = 0; batchIndex < contractBatches.length; batchIndex++) {
        ...
        console.log(
          `[BATCH] Batch ${batchIndex + 1}/${contractBatches.length} (${batch.length} contracts)`
        )
        ...
        try {
          ...
          if (largeTrades.length > 0) {
            console.log(
              `[BATCH] Batch ${batchIndex + 1}: Found ${largeTrades.length} large trades, ${batchTrades.length} total trades`
            )
          }
      ...
      console.log(
        `[OK] ${ticker}: Found ${allTrades.length} trades across ${contractsWithTrades} active contracts`
      )
      return allTrades
    } catch (error) {
  ...
  async fetchOptionsSnapshotRobust(ticker: string): Promise<ProcessedTrade[]> {
    ...
    try {
      ...
      console.log(
        `[OK] ${ticker}: ${validContracts} valid contracts, ${filteredOut} deep ITM filtered out`
      )
  ...
  async fetchLiveStreamingTrades(ticker: string): Promise<ProcessedTrade[]> {
    ...
    try {
      ...
      console.log(
        `[SCAN] Processing ALL ${allContracts.length} active contracts for ${ticker} (all expirations)...`
      )
      ...
      liveTradesResults.sort(
        (a, b) => new Date(b.trade_timestamp).getTime() - new Date(a.trade_timestamp).getTime()
      )

      console.log(
        `[LIVE] LIVE RESULT: Found ${liveTradesResults.length} real-time trades for ${ticker} from today`
      )
    ...
    try {
      ...
      for (const contract of topContracts) {
        try {
          ...
          todaysTrades.push(
            ...todaysContractTrades.map((trade) => ({
              ...trade,
              ticker: contract.ticker,
              underlying_ticker: ticker,
  ...
  async fetchOptionsSnapshotFast(ticker: string): Promise<ProcessedTrade[]> {
    ...
    try {
      ...
      console.log(
        `[OK] ${ticker}: Filtered to ${filteredContracts.length} contracts (within 5% ITM rule)`
      )

      // [OPT] OPTIMIZED BULK PROCESSING: Use snapshots + parallel batching
      const trades = await this.fetchBulkOptionsTradesOptimized(
        filteredContracts,
        ticker,
        currentPrice,
        todayStr
  ...
  private async getCurrentStockPrice(ticker: string): Promise<number> {
    try {
      ...
      console.warn(
        `[ERROR] Failed to get current price for ${ticker}:`,
  ...
  private async getHistoricalSpotPrice(ticker: string, timestamp: number): Promise<number> {
    try {
      ...
      if (timestamp < oneYearAgo || timestamp > now) {
        console.warn(
          `[WARN] Invalid timestamp for ${ticker}: ${new Date(timestamp).toISOString()}, using current price`
        )
      ...
      if (data.results && data.results.length > 0) {
        ...
        if (closestBar) {
          ...
          console.log(
            `[OK] Historical spot price for ${ticker} at ${tradeDate.toLocaleString()}: $${closestBar.c}`
          )
          return closestBar.c
        }
      }
      ...
      console.log(
        `[WARN] Could not find historical data for ${ticker} at ${tradeDate.toLocaleString()}, using current price`
      )
      return await this.getCurrentStockPrice(ticker)
    } catch (error) {
  ...
  async processRawTradesData(
    rawTrades: OptionsTradeData[],
    requestedTicker?: string
  ): Promise<ProcessedTrade[]> {
    console.log(
      `[PROC] Processing ${rawTrades.length} raw trades for ${requestedTicker || 'ALL'} tickers`
    )
  ...
  private async convertRawToProcessed(rawTrade: OptionsTradeData): Promise<ProcessedTrade | null> {
    ...
    const tradeDate = new Date(tradeTimestamp)
    const daysToExpiry = Math.ceil(
      (expiryDate.getTime() - tradeDate.getTime()) / (1000 * 60 * 60 * 24)
    )
  ...
  private async performSweepScan(ticker: string): Promise<ProcessedTrade[]> {
    try {
      ...
      console.log(
        `[OK] ${ticker} @ $${stockPrice}: Scanning ${strikes.length} strikes from $${minStrike.toFixed(2)} to $${maxStrike.toFixed(2)} (all increments)`
      )
      ...
      for (const exp of expirations) {
        for (const strike of strikes) {
          for (const type of ['C', 'P']) {
            ...
            const contractPromise = this.fetchContractTrades(
              optionTicker,
              strike,
              exp,
              type === 'C' ? 'call' : 'put',
      ...
      console.log(
        `[BATCH] Processing ${contractPromises.length} contracts concurrently for ${ticker}...`
      )
      ...
      console.log(
        `[READY] Processed ALL ${contractPromises.length} contracts in parallel for ${ticker}`
      )

      console.log(
        `[INFO] Found ${allTrades.length} total trades, classifying as SWEEPS/BLOCKS/MINIS...`
      )
      ...
      console.log(
        `[INFO] Final Classification: ${sweepCount} sweeps, ${blockCount} blocks, ${miniCount} minis`
      )
  ...
  private classifyAllTrades(allTrades: any[]): ProcessedTrade[] {
    ...
    const convertedTrades = uniqueTrades.map((trade) => {
      ...
      return {
        ...
        conditions: trade.conditions || [],
        trade_timestamp:
          trade.trade_timestamp instanceof Date ? trade.trade_timestamp : new Date(trade.timestamp),
  ...
  private getAllExpirations(count: number): string[] {
    ...
    for (let i = 0; i < 365 && expirations.length < count; i++) {
      // 1 year = 365 days
  ...
  private async enrichTradesInstantlyParallel(trades: ProcessedTrade[]): Promise<ProcessedTrade[]> {
    ...
    console.log(
      `[ENRICH] BACKEND ENRICHMENT: Processing ${trades.length} trades with combined snapshot approach`
    )
    ...
    console.log(
      `[OK] Enriching ${trades.length} trades across ${tradesByTicker.size} tickers in parallel`
    )

    // Process all tickers in parallel for maximum speed
    const enrichmentPromises = Array.from(tradesByTicker.entries()).map(
      async ([ticker, tickerTrades], idx) => {
        try {
            ...
            tickerTrades.map(async (trade, tradeIdx) => {
              try {
                ...
                if (data.results) {
                  ...
                  const tradeTimestampNs =
                    (trade as any).sip_timestamp ||
                    new Date((trade as any).trade_timestamp).getTime() * 1_000_000
                  const historicalQuote = await this.fetchHistoricalBidAsk(
                    optionTicker,
                    tradeTimestampNs
                  )
                ...
                console.warn(
                  `[WARN] Failed to enrich ${trade.ticker}:`,
    ...
    console.log(
      `[OK] Backend enrichment complete: ${allEnrichedTrades.length} trades fully enriched`
    )
        ...
        try {
          ...
          return Promise.all(
            tickerTrades.map(async (trade) => {
              ...
              const tradeTimestampNs =
                trade.sip_timestamp || new Date(trade.trade_timestamp as any).getTime() * 1_000_000
              const historicalQuote = await this.fetchHistoricalBidAsk(
                trade.ticker,
                tradeTimestampNs
              )
  ...
  private async fetchBatchedContractTrades(
    ...
    try {
      ...
      console.log(
        `[OK] ${ticker}: Snapshot loaded with enrichment data for ${enrichmentMap.size} contracts`
      )
          ...
          for (const trade of tradesData.results) {
            ...
            if (contractTrades.length === 0) {
              console.log(`[OK] Sample enriched trade for ${contract.ticker}:`, {
                ...
                classification,
                spotPrice,
                currentPrice: enrichment?.current_price
                  ? enrichment.current_price / 100
                  : trade.price / 100,
              })
            }
  ...
  private async fetchHistoricalBidAsk(
    optionTicker: string,
    timestampNs: number
  ...
  private async fetchAllContractsPaginated(ticker: string): Promise<any[]> {
    ...
    do {
      ...
      try {
        ...
        if (data.results && data.results.length > 0) {
          ...
          console.log(
            `[PAGE] Page ${pageCount}: +${data.results.length} contracts (Total: ${allContracts.length})`
          )
        }
    ...
    console.log(
      `[OK] Pagination complete: ${allContracts.length} total contracts from ${pageCount} pages`
    )
    return allContracts
  }
  ...
  private async fetchBulkOptionsTradesOptimized(
    ...
    console.log(
      `[BULK] BULK OPTIMIZATION: Processing ${contracts.length} contracts for ${ticker} with parallel snapshots`
    )
    ...
    console.log(
      `[SPEED] MAXIMUM SPEED: ${contractBatches.length} batches of ${SNAPSHOT_BATCH_SIZE} contracts each`
    )
    ...
    const batchPromises = contractBatches.map(async (batch, batchIndex) => {
      ...
      try {
        ...
        if (!snapshotResponse.ok) {
          console.warn(
            `[WARN] Snapshot failed for batch ${batchIndex + 1}: ${snapshotResponse.status}`
          )
          return []
        }
    ...
    console.log(
      `[MULTI] BULK OPTIMIZATION COMPLETE: ${allTrades.length} trades from ${contracts.length} contracts for ${ticker}`
    )
    return allTrades
  }
  ...
  async enrichTradesWithVolOIParallel(trades: ProcessedTrade[]): Promise<ProcessedTrade[]> {
    ...
    console.log(
      `[ENRICH] SEQUENTIAL ENRICHMENT: ${trades.length} trades in ${batches.length} batches`
    )
            ...
            try {
              ...
              if (data.status === 'OK' && data.results) {
                  ...
                  if (quoteResponse.ok) {
                    ...
                    if (
                      ...
                      if (batchIndex === 0 && batch.indexOf(trade) === 0) {
                        ...
                        console.log(
                          `  Quote time: ${new Date(quoteData.results[0].sip_timestamp / 1000000).toISOString()}`
                        )
                        console.log(`  Bid: $${bid}, Ask: $${ask}`)
                      }
                    } else {
                      console.log(
                        `[WARN] No historical quote found for ${optionTicker} at ${tradeDate.toISOString()}`
                      )
                    }
                  }
                } catch (quoteError) {
                ...
                return {
                  ...
                  open_interest: openInterest,
                  vol_oi_ratio:
                    openInterest && openInterest > 0 && volume ? volume / openInterest : undefined,
                  fill_style: fillStyle,
                  bid,
                  ask,
  ...
  async enrichTradesWithHistoricalVolOI(trades: ProcessedTrade[]): Promise<ProcessedTrade[]> {
    ...
    console.log(
      `[HIST] HISTORICAL VOL/OI ENRICHMENT: ${trades.length} trades in ${batches.length} batches`
    )
    ...
    const enrichedBatches = await Promise.all(
      batches.map(async (batch, batchIndex) => {
          ...
          batch.map(async (trade) => {
            try {
              ...
              if (snapData.status === 'OK' && snapData.results) {
                ...
                if (!snapshot) {
                  console.log(
                    `[WARN] No snapshot found for ${optionTicker} in enrichTradesWithHistoricalVolOI`
                  )
                  return trade
                }
        ...
        if (batchIndex % 10 === 0) {
          console.log(
            `[BATCH] Historical enrichment batch ${batchIndex + 1}/${batches.length} complete`
          )
        }
