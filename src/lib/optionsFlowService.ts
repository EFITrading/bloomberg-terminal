...
export function getTodaysMarketOpenTimestamp(): number {
 try {
 // We want the most recent market open (which could be today or Friday if it's weekend)
 const now = new Date();
 
 // Get the current date components (we'll use these as the base)
 let year = now.getFullYear();
 let month = now.getMonth();
 let day = now.getDate();
 
 // Check day of week to determine the trading day
 const dayOfWeek = now.getDay();
 
 // If it's Sunday (0), we want Friday's market open
 if (dayOfWeek === 0) {
 day -= 2; // Go back to Friday
 }
 // If it's Saturday (6), we want Friday's market open 
 else if (dayOfWeek === 6) {
 day -= 1; // Go back to Friday
 }
 // For weekdays, use today's date (market hours consideration handled elsewhere)
 
 // Handle month/year rollover if day goes negative
 if (day <= 0) {
 month -= 1;
 if (month < 0) {
 month = 11;
 ...
 // Create the market open time: 9:30 AM Eastern on the determined date
 // We'll create this in UTC directly to avoid timezone confusion
 
 // October 20, 2025 is in Daylight Saving Time, so Eastern is UTC-4
 const targetDate = new Date(year, month, day);
 const isDST = isDateInDST(targetDate);
 const utcOffset = isDST ? 4 : 5; // Hours that Eastern is behind UTC
 
 // Create 9:30 AM Eastern as UTC time
 // If Eastern is UTC-4, then 9:30 AM Eastern = 13:30 UTC
 // If Eastern is UTC-5, then 9:30 AM Eastern = 14:30 UTC
 const marketOpenUTC = new Date(Date.UTC(year, month, day, 9 + utcOffset, 30, 0, 0));
 
 const finalTimestamp = marketOpenUTC.getTime();
 ...
 // Simple fallback: today at 9:30 AM local time
...
function isDateInDST(date: Date): boolean {
 ...
 const secondSundayMarch = new Date(year, 2, 8 + (7 - march.getDay()) % 7);
...
export class OptionsFlowService {
 ...
 // Classify all trades after collection to enable proper SWEEP/BLOCK/MINI detection
 ...
 console.error(` Fallback error for ${ticker}:`, tickerError);
 }
 }
 ...
 console.log(` WARNING: Only scanning 1 ticker: ${tickersToScan[0]} - this suggests the 'ALL' logic failed`);
 }
 ...
 console.log(` ${symbol}: No live trades yet today - this is normal early in trading`);
 }
 } else {
 ...
 if (!response.ok) {
 console.warn(` Failed to fetch ${ticker} snapshot: ${response.status}`);
 return [];
 }
 ...
 console.warn(`⏱ Rate limited for ${optionTicker}`);
 } else {
 console.warn(` Error fetching trades for ${optionTicker}: ${errorMessage}`);
 }
 ...
 console.log(` After trade type classification: ${filtered.length} trades`);
 ...
 console.error(`⏱ HTTP 429 Rate Limited - Implementing exponential backoff`);
 if (attempt < maxRetries) {
 ...
 const BATCH_SIZE = 250; const contractBatches = this.chunkArray(validContracts, BATCH_SIZE);
 ...
 if (!response.ok) {
 console.warn(` Price API error for ${ticker}: ${response.status}`);
 ...
 console.warn(` No valid price data for ${ticker}, using fallback`);
 return 100;
 }
 } catch (error) {
 ...
 console.warn(` Invalid timestamp for ${ticker}: ${new Date(timestamp).toISOString()}, using current price`);
 ...
 console.log(` Could not find historical data for ${ticker} at ${tradeDate.toLocaleString()}, using current price`);
 return await this.getCurrentStockPrice(ticker);
 } catch (error) {
 ...
 console.log(' No raw trades to process');
 return [];
 }
 ...
 */
 ...
 console.log(` Trade already classified: ${trade.ticker} - ${trade.trade_type}`);
 return trade as ProcessedTrade;
 }
 
 // Convert parallel processing trade to ProcessedTrade format
 console.log(` Converting parallel processed trade: ${trade.ticker || trade.option_ticker} - $${trade.total_premium}`);
 return {
 ...
 if (timestampMs > now) {
 console.warn(` Future timestamp detected for ${contract.ticker}, using current time`);
 ...
 console.warn(`⏱ Rate limited for ${contract.ticker}, continuing with next...`);
 } else {
 ...
 if (pageCount > 50) {
 console.warn(` Reached pagination limit (50 pages) for ${ticker}`);
 break;
 }
 ...
 console.warn(` Snapshot failed for batch ${batchIndex + 1}: ${snapshotResponse.status}`);
 return [];
 }
