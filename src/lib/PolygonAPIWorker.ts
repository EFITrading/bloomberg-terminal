...
class PolygonAPIWorker {
    ...
    */
    ...
    private async executeRequest(request: any): Promise<void> {
        ...
        try {
            // Rewrite polygon.io URLs through server-side proxy so the key never reaches the client
            let fetchUrl = request.url
            if (fetchUrl.includes('api.polygon.io/')) {
                const u = new URL(fetchUrl)
                u.searchParams.delete('apiKey')
                u.searchParams.delete('apikey')
                const proxyPath = u.pathname.replace(/^\//, '')
                fetchUrl = `/api/polygon/${proxyPath}?${u.searchParams.toString()}`
            }
