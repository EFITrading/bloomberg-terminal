...
class DataPreloaderService {
 ...
 start(): void {
 if (this.isRunning) {
 console.log(' Preloader already running');
 return;
 }
 ...
 stop(): void {
 if (!this.isRunning) {
 console.log(' Preloader not running');
 return;
 }
 ...
 ]).catch(err => console.warn(` Background preload failed for ${symbol}:`, err));
 }, 100);
 ...
 console.log(' Preloader configuration updated');
