...
class QuickSeasonalService {
  ...
  async getQuickSeasonalData(symbol: string, yearsBack: number = 15): Promise<QuickSeasonalData | null> {
    try {
      ...
      console.log(` FAST processing: ${startDateStr} to ${endDateStr} (fetching ${yearsToFetch} years max, will use ${yearsBack})`);
      ...
      if (yearsBack && historicalResponse.results.length > 0) {
        const dataEndDate = new Date(historicalResponse.results[historicalResponse.results.length - 1].t);
