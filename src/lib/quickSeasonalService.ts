...
class QuickSeasonalService {
  ...
  private analyze30DayPatterns(dailyData: DailySeasonalData[]) {
    ...
    for (let startDay = 1; startDay <= 365 - windowSize; startDay++) {
      ...
      if (validDays >= 20) {
        // Ensure we have enough data points (reduced from 25 for performance)
  ...
  async getQuickSeasonalData(
    symbol: string,
    yearsBack: number = 15
  ): Promise<QuickSeasonalData | null> {
    try {
      console.log(
        ` QuickSeasonalService: Starting FAST analysis for ${symbol} with ${yearsBack} years`
      )
      ...
      console.log(
        ` FAST processing: ${startDateStr} to ${endDateStr} (fetching ${yearsToFetch} years max, will use ${yearsBack})`
      )
      ...
      if (cachedHistorical && cachedTicker) {
        ...
        console.log(
          ` Historical data results: ${symbol}=${historicalResponse?.resultsCount || 0} points (SPY skipped for speed)`
        )

        tickerDetails = await polygonService.getTickerDetails(symbol)
        console.log(
          ` Ticker details: ${tickerDetails ? `${tickerDetails.name} (${tickerDetails.ticker})` : 'not found'}`
        )
      ...
      if (
        ...
        historicalResponse.results.length === 0
      ) {
        console.error(
          ` No historical data for ${symbol}: response=${!!historicalResponse}, results=${historicalResponse?.results?.length || 0}`
        )
        return null
      }

      console.log(
        ` Processing ${historicalResponse.results.length} data points for seasonal analysis...`
      )
      ...
      if (yearsBack && historicalResponse.results.length > 0) {
        ...
        cutoffDate.setFullYear(dataEndDate.getFullYear() - yearsBack)
        ...
        console.log(` Filtered to ${yearsBack} years: ${filteredData.length} data points`)
      }
      ...
      if (seasonalData) {
        console.log(
          ` Seasonal analysis complete for ${symbol}: ${seasonalData.yearsOfData} years, win rate ${seasonalData.winRate.toFixed(1)}%`
        )
      } else {
  ...
  private processDailySeasonalData(
    ...
    console.log(
      ` FAST seasonal analysis complete for ${symbol}: ${years} years, win rate ${winRate.toFixed(1)}%`
    )
