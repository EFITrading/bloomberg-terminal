...
async function isMarketHolidayAsync(
  date: Date
): Promise<{ isClosed: boolean; holidayName?: string }> {
...
export class AlmanacService {
  ...
  async getMonthlySeasonalData(
    ...
    for (const index of INDICES) {
      try {
        const dailyData = await this.calculateDailySeasonalPattern(
          index.symbol,
          month,
          startYear,
          currentYear
  ...
  async getSingleStockMonthlyData(
    symbol: string,
    month: number,
    yearsBack: number = 25
  ): Promise<IndexSeasonalData[]> {
  ...
  private async calculateDailySeasonalPattern(
    ...
    try {
      ...
      for (let tradingDay = 1; tradingDay <= maxTradingDay; tradingDay++) {
        ...
        const postElectionCumulative =
          postElectionReturns.length > 0
  ...
  async getMonthlyVitalStats(month: number): Promise<MonthlyVitalStats[]> {
    ...
    for (const index of INDICES) {
      try {
        const stat = await this.calculateMonthlyStats(
          index.symbol,
          index.name,
          month,
  ...
  private async calculateMonthlyStats(
    ...
    try {
      ...
      const postElectionReturns = monthlyReturns
        .filter((r) => r.isPostElection)
        .map((r) => r.return)
      ...
      const postElectionAvg =
        postElectionReturns.length > 0
          ? postElectionReturns.reduce((a, b) => a + b, 0) / postElectionReturns.length
          : avgChange

      const bestYearData = monthlyReturns.reduce(
        (best, curr) => (curr.return > best.return ? curr : best),
        { year: 0, return: -Infinity }
      )
      const worstYearData = monthlyReturns.reduce(
        (worst, curr) => (curr.return < worst.return ? curr : worst),
        { year: 0, return: Infinity }
      )
  ...
  async getMonthlyCalendar(
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      const stats =
        !isMarketClosed && dailyStats[tradingDayCount]
  ...
  private async getDailyHistoricalStats(
    month: number
  ): Promise<{ [tradingDay: number]: { winRate: number; avgReturn: number } }> {
  ...
  async getMarketAtAGlance(): Promise<MarketGlanceData> {
    ...
    return {
      asOfDate: today.toLocaleDateString('en-US', {
        month: 'long',
        day: 'numeric',
        year: 'numeric',
      }),
      ...
      seasonal: {
        outlook:
          seasonalData.winRate >= 60
            ? 'Bullish'
            : seasonalData.winRate <= 40
              ? 'Bearish'
              : 'Neutral',
  ...
  private async calculateMonthlySeasonalStats(
    month: number
  ): Promise<{ winRate: number; avgReturn: number }> {
    try {
      const currentYear = new Date().getFullYear()
      const startDate = `${currentYear - 21}-01-01`
      const endDate = `${currentYear}-12-31`
