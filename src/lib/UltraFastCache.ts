// Ultra-fast memory cache with intelligent invalidation
...
class UltraFastDataCache {
  ...
  invalidateSymbol(symbol: string, dataType?: string): void {
    ...
    console.log(`🗑️ Cache INVALIDATED: ${keysToDelete.length} entries for ${symbol}${dataType ? ` (${dataType})` : ''}`);
  }
  ...
  private evictLeastUsed(): void {
    ...
    console.log(`🗑️ Cache eviction: removed ${toRemove} least used entries`);
  }
