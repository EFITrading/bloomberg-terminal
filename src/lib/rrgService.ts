...
class RRGService {
  ...
  ): Array<{ date: string; rsRatio: number }> {
  ...
  private calculateJdKRSRatio(
    rsData: Array<{ date: string; rsRatio: number }>,
    rsPeriod: number = 10,
    longPeriod: number = 26
  ): Array<{ date: string; rsRatio: number; normalizedRS: number }> {
    ...
    for (let i = finalStartIndex; i < rsData.length; i++) {
      ...
      if (
        rsTrend[i] !== undefined &&
        rsTrendSMA[trendIndex] !== undefined &&
        rsTrendStdDev[trendIndex] !== undefined
      ) {
  ...
  private calculateRSMomentum(
    normalizedRSData: Array<{ date: string; normalizedRS: number }>,
    momentumPeriod: number = 10
  ): Array<{ date: string; normalizedRS: number; rsMomentum: number }> {
  ...
  public async calculateSectorRRG(
    ...
    try {
      ...
      const benchmarkPrices = await this.getHistoricalPrices(
        BENCHMARK_SYMBOL,
        fromDate,
        toDate,
        interval
      ...
      for (const etf of SECTOR_ETFS) {
        try {
          ...
          const sectorPrices = await this.getHistoricalPrices(
            etf.symbol,
            fromDate,
            toDate,
            interval
          ...
          if (rsData.length < minDataPoints) {
            console.warn(
              ` Insufficient data for ${etf.symbol} (need ${minDataPoints}, got ${rsData.length})`
            )
            continue
          }
  ...
  public async calculateCustomRRG(
    ...
    const stockPromises = symbols.map(async (symbol) => {
      try {
        ...
        const latest = rsMomentumData[rsMomentumData.length - 1]
        const tail = rsMomentumData.slice(-tailLength - 1, -1).map((point) => ({
          rsRatio: point.normalizedRS,
          rsMomentum: point.rsMomentum,
          date: point.date,
        }))
