...
class PremiumImbalanceScanner {
  ...
  private async analyzeSymbol(
    ...
    // Check if stock price is close enough to midpoint between strikes
    if (!this.isStockAtMidpoint(stockPrice, strikeSpacing)) {
      return null;
    }
  ...
  private isStockAtMidpoint(stockPrice: number, strikeSpacing: number): boolean {
    // Find the nearest strike below and above the stock price
    const lowerStrike = Math.floor(stockPrice / strikeSpacing) * strikeSpacing;
    const upperStrike = lowerStrike + strikeSpacing;
    ...
    else if (strikeSpacing >= 2.5) tolerance = 0.1; // $2.5 apart -> $0.1 tolerance (corrected from your example)
