...
class ParallelOptionsFlowProcessor {
  constructor() {
    ...
    console.log(
      `[PARALLEL] PARALLEL PROCESSOR: ${this.numWorkers} workers available (${os.cpus().length} CPU cores)`
    )
  ...
  async processTickersInParallel(tickers, optionsFlowService, onProgress, dateRange) {
    ...
    console.log(
      `[PARALLEL] PARALLEL: Processing ${tickers.length} tickers across ${this.numWorkers} workers`
    )
    ...
    console.log(
      `[BATCH] OPTIMAL DISTRIBUTION: ${tickers.length} tickers / ${actualWorkers} workers = ${optimalBatchSize} tickers per worker`
    )
    ...
    console.log(
      `[BATCH] Split into ${batches.length} batches across ${actualWorkers} workers (${optimalBatchSize} tickers each)`
    )
    ...
    if (onProgress) {
      const totalTrades = results.reduce((sum, r) => sum + r.length, 0)
      onProgress(
        [],
        `[SERVER] All ${results.length} workers done. ${totalTrades} raw trades collected. Classifying...`
      )
    }
    ...
    this.benchmarks.bottlenecks.set(
      'WORKER_CREATION_PHASE',
      workerCreationEnd - workerCreationStart
    )
  ...
  createWorkerPromise(batch, workerIndex, onProgress, dateRange) {
    return new Promise((resolve) => {
      ...
      if (onProgress) {
        ...
        onProgress(
          [],
          `Worker ${workerIndex} processing ${batch.length} tickers (batch ${workerIndex + 1}/${totalBatches})...`,
          {
            current: workerIndex,
            total: totalBatches,
      ...
      worker.on('message', (result) => {
        ...
        if (result.type === 'trades_found') {
          ...
          console.log(
            `[OK] Worker ${workerIndex}: Found ${result.trades.length} trades from ${result.ticker} ${result.contract} (${allWorkerTrades.length} total)`
          )
          ...
          if (onProgress && result.trades.length > 0) {
            onProgress(
              result.trades,
              `[LIVE] LIVE: Found ${result.trades.length} trades from ${result.ticker}`,
              {
                worker: workerIndex,
          ...
          if (currentProcessing) {
            ...
            console.log(
              `[OK] Worker ${workerIndex}: Completed ${result.processedTickers} tickers in ${totalTime.toFixed(2)}ms, streamed ${result.totalTradesStreamed} trades (${currentProcessing.apiCalls} API calls)`
            )
            ...
            console.log(
              `[OK] Worker ${workerIndex}: Completed - streamed ${result.totalTradesStreamed} trades`
            )
          }
          ...
          allWorkerTrades.push(...result.trades)
          console.log(
            `[CHUNK] Worker ${workerIndex}: Received chunk ${result.chunkInfo.current}/${result.chunkInfo.total} (${result.trades.length} trades, total: ${allWorkerTrades.length})`
          )
          ...
          if (onProgress && result.trades.length > 0) {
            onProgress(
              result.trades,
              `Worker ${workerIndex}: Processing chunk ${result.chunkInfo.current}/${result.chunkInfo.total}`,
              {
                worker: workerIndex,
                newTrades: result.trades.length,
                workerTotal: allWorkerTrades.length,
          ...
          if (result.chunkInfo.isLast) {
            ...
            if (currentProcessing) {
              ...
              console.log(
                `[OK] Worker ${workerIndex}: Completed ${currentProcessing.batchSize} tickers in ${totalTime.toFixed(2)}ms, found ${allWorkerTrades.length} trades (${currentProcessing.apiCalls} API calls)`
              )
          ...
          if (currentProcessing) {
            ...
            console.log(
              `[OK] Worker ${workerIndex}: Completed ${currentProcessing.batchSize} tickers in ${totalTime.toFixed(2)}ms, found ${currentProcessing.tradesFound} trades (${currentProcessing.apiCalls} API calls)`
            )
            ...
            console.log(
              `[OK] Worker ${workerIndex}: Completed batch - found ${result.trades.length} trades from ${batch.length} tickers`
            )
          }
      ...
      worker.on('error', (error) => {
        ...
        if (onProgress) {
          onProgress([], `[SERVER] Worker ${workerIndex} CRASHED: ${error.message}`)
        }
        resolve(allWorkerTrades) // Return whatever we got so far
      })
      ...
      worker.on('exit', (code) => {
        if (code !== 0) {
          ...
          if (onProgress) {
            onProgress(
              [],
              `[SERVER] Worker ${workerIndex} exited with code ${code} (had ${allWorkerTrades.length} trades)`
            )
          }
          resolve(allWorkerTrades) // Return whatever we got so far
        } else {
          if (onProgress) {
            onProgress(
              [],
              `[SERVER] Worker ${workerIndex} exited cleanly with ${allWorkerTrades.length} trades`
            )
          }
        }
      })
  ...
  displayPerformanceReport() {
    ...
    const totalOperationsTime =
      this.benchmarks.totalOperations.get('endTime') -
      this.benchmarks.totalOperations.get('startTime')
    ...
    if (this.benchmarks.workerCreation.size > 0) {
      ...
      console.log(
        `   Average: ${avgCreationTime.toFixed(2)}ms | Max: ${maxCreationTime.toFixed(2)}ms | Count: ${creationTimes.length}`
      )
    }
    ...
    if (this.benchmarks.workerCompletion.size > 0) {
      ...
      if (successfulWorkers.length > 0) {
        ...
        console.log(
          `   Processing Time - Avg: ${avgProcessingTime.toFixed(2)}ms | Min: ${minProcessingTime.toFixed(2)}ms | Max: ${maxProcessingTime.toFixed(2)}ms`
        )
        ...
        console.log(
          `   Trades per Second: ${(totalTrades / (totalOperationsTime / 1000)).toFixed(2)}`
        )
        console.log(
          `   API Calls per Second: ${(totalApiCalls / (totalOperationsTime / 1000)).toFixed(2)}`
        )
      }
    ...
    if (this.benchmarks.bottlenecks.size > 0) {
      ...
      const slowestPhase = Array.from(this.benchmarks.bottlenecks.entries()).reduce(
    ...
    if (this.benchmarks.workerCompletion.size > 0) {
      const completions = Array.from(this.benchmarks.workerCompletion.values()).filter(
        (w) => w.status === 'success'
      )
      if (completions.length > 0) {
        ...
        if (variance > avgTime * 0.5) {
          console.log(
            '[NOTE] High variance in worker completion times - consider load balancing optimization'
          )
        }

        if (avgTime > 5000) {
          console.log(
            '[NOTE] Worker processing time is high - consider GPU acceleration for calculations'
          )
        }
