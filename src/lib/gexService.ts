...
class GEXService {
  ...
  private async getOptionsChain(symbol: string): Promise<any[]> {
    throw new Error(`Options chain data unavailable for ${symbol} `)
  }
   ...
   */
  private calculateGammaForStrike(
    strike: number,
    currentPrice: number,
    optionType: 'call' | 'put'
  ): number {
  ...
  private determineMarketRegime(
    data: GEXDataPoint[],
    currentPrice: number
  ): 'positive_gamma' | 'negative_gamma' | 'mixed' {
    // Find the data point closest to current price
    const currentGEX = data.reduce((closest, point) =>
      Math.abs(point.price - currentPrice) < Math.abs(closest.price - currentPrice)
        ? point
        : closest
    )

    if (currentGEX.netGamma > 1000000) {
      // Large positive gamma
      return 'positive_gamma'
    } else if (currentGEX.netGamma < -1000000) {
