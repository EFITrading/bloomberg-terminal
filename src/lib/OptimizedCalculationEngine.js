...
class OptimizedCalculationEngine {
  ...
  initializeCompiledExpressions() {
    ...
    this.compiledExpressions.set(
      'd1',
      math.compile('(log(S / K) + (r + (sigma^2) / 2) * T) / (sigma * sqrt(T))')
    )
    ...
    this.compiledExpressions.set(
      'theta_call',
      math.compile('-(S * normpdf(d1) * sigma) / (2 * sqrt(T)) - r * K * exp(-r * T) * normcdf(d2)')
    )
    ...
    this.compiledExpressions.set(
      'volatility',
      math.compile('sqrt(mean((returns - mean(returns)).^2))')
    )
  ...
  calculateBatch(calculations, useSharedBuffer = true) {
    ...
    try {
      ...
      console.log(
        ` Batch calculation complete: ${calculations.length} operations in ${batchTime.toFixed(2)}ms (${cacheHitsThisBatch} cache hits)`
      )
  ...
  calculateWithSharedBuffer(calculations) {
    ...
    try {
      ...
      console.log(
        ` SharedArrayBuffer allocated: ${(resultSize / 1024).toFixed(2)}KB for ${numCalculations} calculations`
      )
      ...
      console.log(
        ` SharedArrayBuffer calculation complete: ${numCalculations} operations in ${bufferTime.toFixed(2)}ms`
      )
  ...
  generateCacheKey(calc) {
    return JSON.stringify({
      type: calc.type,
      params: Object.keys(calc.params)
        .sort()
        .reduce((obj, key) => {
          obj[key] =
            typeof calc.params[key] === 'number'
