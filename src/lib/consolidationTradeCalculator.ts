...
export class ConsolidationTradeCalculator {
   ...
   */
  private async getTargetExpiration(
    symbol: string,
    period: '5-DAY' | '13-DAY'
  ): Promise<{ expiration: string; daysOut: number } | null> {
    try {
   ...
   */
  private async getOptionQuote(
    underlying: string,
    optionTicker: string
  ): Promise<{ bid: number; ask: number; last: number; iv: number } | null> {
    try {
  ...
  async calculateTradeSetup(
    ...
    try {
      ...
      const callStrikes = chain.calls
        .map((c: any) => c.strike_price)
        .sort((a: number, b: number) => a - b)
      const putStrikes = chain.puts
        .map((p: any) => p.strike_price)
        .sort((a: number, b: number) => a - b)
      ...
      const callTarget1Premium = this.calculateBSPrice(
        callTarget1Stock,
        callStrike,
        T * 0.7,
        this.RISK_FREE_RATE,
      ...
      const callTarget2Premium = this.calculateBSPrice(
        callTarget2Stock,
        callStrike,
        T * 0.5,
        this.RISK_FREE_RATE,
      ...
      const putTarget1Premium = this.calculateBSPrice(
        putTarget1Stock,
        putStrike,
        T * 0.7,
        this.RISK_FREE_RATE,
      ...
      const putTarget2Premium = this.calculateBSPrice(
        putTarget2Stock,
        putStrike,
        T * 0.5,
        this.RISK_FREE_RATE,
