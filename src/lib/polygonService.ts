...
class PolygonService {
  ...
  private async makeRequest<T>(endpoint: string): Promise<T | null> {
    ...
    try {
      ...
      try {
        ...
        throw new Error(
          `Invalid JSON response from Polygon API: ${parseError instanceof Error ? parseError.message : 'Unknown error'}`
        )
      }
  ...
  async getTickerDetails(symbol: string): Promise<PolygonTickerData | null> {
    try {
      ...
      if (!response.ok) {
        ...
        console.warn(
          ` Ticker details API error for ${symbol}: ${errorData.error || `${response.status} ${response.statusText}`}`
        )
        return null
      }
  ...
  async getHistoricalData(
    ...
    try {
      ...
      if (!response.ok) {
        ...
        const errorData = await response.json().catch(() => ({ error: 'Unknown error' }))
        console.warn(
          ` Historical data API error for ${symbol}: ${errorData.error || `${response.status} ${response.statusText}`}`
        )
        return null
      }
  ...
  async getBulkHistoricalData(
    ...
    return withCircuitBreaker('historicalData', async () => {
      ...
      for (let attempt = 1; attempt <= maxRetries; attempt++) {
        try {
          ...
          if (!response.ok) {
            ...
            console.warn(
              ` Bulk historical data API error for ${symbol} (attempt ${attempt}/${maxRetries}): ${errorMsg}`
            )
          ...
          if (!data.results) {
            console.error(
              ` Missing 'results' field for ${symbol}. Response keys: ${Object.keys(data).join(', ')}`
            )
            return null
          }
          ...
          if (data.results.length === 0) {
            console.error(
              ` EMPTY results array for ${symbol}. Status: ${data.status}, Query: ${startDateStr} to ${endDateStr}`
            )
            return null
          }
          ...
          if (error instanceof Error) {
            if (error.name === 'AbortError') {
              ...
              console.warn(
                ` Connection error for ${symbol} (attempt ${attempt}/${maxRetries}): ${error.message}`
              )
            } else {
              console.warn(
                ` Error fetching bulk data for ${symbol} (attempt ${attempt}/${maxRetries}): ${error.message}`
              )
            }
          }
      ...
      console.error(
        ` Failed to get bulk historical data for ${symbol} after ${maxRetries} attempts. Last error:`,
        lastError?.message
      )
      return null
    })
  ...
  private calculateSeasonalPattern(priceData: any[], symbol: string): SeasonalPattern | null {
    try {
      ...
      return {
        ...
        riskLevel:
          Math.abs(bestAvgReturn) > 5 ? 'High' : Math.abs(bestAvgReturn) > 2 ? 'Medium' : 'Low',
