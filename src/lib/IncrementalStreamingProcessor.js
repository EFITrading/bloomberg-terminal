...
class IncrementalStreamingProcessor {
  ...
  async processTickersWithStreaming(tickers, onProgress, onIncrementalResult) {
    ...
    const workerPromises = batches.map((batch, workerIndex) => {
      return this.createStreamingWorkerPromise(batch, workerIndex, onProgress, (streamData) => {
        ...
        console.log(
          ` Stream ${totalStreamingEvents}: Worker ${workerIndex} sent ${streamData.results.length} results (${streamData.completedTickers}/${streamData.totalTickers} tickers complete)`
        )
    ...
    const streamingLatency = streamingMetrics.firstStreamTime
      ? streamingMetrics.firstStreamTime - streamingStart
      : 0
    ...
    console.log(
      ` Incremental streaming complete: ${finalResults.length} total results from ${totalStreamingEvents} streams`
    )
  ...
  createStreamingWorkerPromise(batch, workerIndex, onProgress, onStream) {
    return new Promise((resolve, reject) => {
        ...
        switch (message.type) {
          ...
          case 'worker_complete':
            ...
            if (streamInfo) {
              ...
              console.log(
                ` Streaming worker ${workerIndex}: Completed ${streamInfo.totalTickers} tickers in ${totalTime.toFixed(2)}ms, sent ${streamInfo.streamCount} streams with ${streamInfo.streamedResults} results`
              )
            }
      ...
      const timeout = setTimeout(() => {
        console.warn(` Streaming worker ${workerIndex} timeout after 30s`)
        streamingActive = false
        worker.terminate()
        this.activeStreams.delete(workerIndex)
        resolve(accumulatedResults)
