...
console.log(
  ` Streaming Worker ${workerIndex}: Started with ${batch.length} tickers, streaming every ${streamingBatchSize} tickers`
)
...
function sendIncrementalStream(results, ticker) {
  ...
  if (
    accumulatedResults.length >= streamingBatchSize ||
    ...
    console.log(
      ` Worker ${workerIndex}: Streamed ${streamResults.length} results (batch ${streamBatch - 1}), ${processedTickers}/${batch.length} tickers complete`
    )
  }
}
...
async function fetchOptionsDataWithStreaming(ticker) {
  ...
  try {
    ...
    console.log(
      ` Worker ${workerIndex}: Scanning ${ticker} in ${timeRange.mode} mode (${timeRange.from} to ${timeRange.to})`
    )
    ...
    console.log(
      ` Worker ${workerIndex}: Found ${contractsResponse.data.results.length} contracts for ${ticker}`
    )

    const allTickerTrades = []
    let contractsProcessed = 0

    // Process contracts in smaller batches for more frequent streaming
    const contractBatchSize = Math.max(
      1,
      Math.min(10, Math.floor(contractsResponse.data.results.length / 5))
    )
      ...
      for (const contract of contractBatch) {
        try {
          ...
          if (tradesResponse.data?.results?.length > 0) {
            ...
            console.log(
              ` Worker ${workerIndex}: Found ${contractTrades.length} trades for ${ticker} ${contract.ticker}`
            )
          ...
          console.error(
            ` Worker ${workerIndex}: Error fetching trades for ${contract.ticker}:`,
    ...
    console.log(
      ` Worker ${workerIndex}: Completed ${ticker} in ${tickerTime.toFixed(2)}ms, found ${allTickerTrades.length} trades from ${contractsProcessed} contracts`
    )
...
async function processTickersWithStreaming() {
  ...
  try {
    ...
    console.log(
      ` Worker ${workerIndex}: Streaming complete - ${totalTradesFound} trades from ${batch.length} tickers in ${totalTime.toFixed(2)}ms via ${streamBatch - 1} streams`
    )
  } catch (error) {
