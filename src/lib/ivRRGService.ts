...
class IVRRGService {
  ...
  private async getHistoricalIV(symbol: string, days: number): Promise<IVData[]> {
    try {
      ...
      console.log(
        `✅ Fetched ${ivData.length} IV data points for ${symbol}. Sample avgIV:`,
        ivData[ivData.length - 1]?.avgIV.toFixed(4)
      )
      return ivData
    } catch (error) {
  ...
  private calculateNormalizedIVRatio(
    ...
    for (let i = finalStartIndex; i < ivRatioData.length; i++) {
      ...
      if (
        ivTrend[i] !== undefined &&
        ivTrendSMA[trendIndex] !== undefined &&
        ivTrendStdDev[trendIndex] !== undefined
      ) {
  ...
  public async calculateIVBasedRRG(
    ...
    try {
      ...
      if (!isSelfBenchmark) {
        ...
        console.log(
          `📊 Using SELF-BENCHMARK mode - each ticker compared to its own historical average`
        )
      }
      ...
      for (const symbol of symbols) {
        ...
        try {
          ...
          if (isSelfBenchmark) {
            ...
            console.log(
              `📊 ${symbol}: Self-benchmark - comparing to own avg IV = ${(avgIV * 100).toFixed(2)}%`
            )
          } else {
          ...
          if (ivRatioData.length < minRequired) {
            console.warn(
              `⚠️ Insufficient data for ${symbol}: have ${ivRatioData.length} points, need ${minRequired} (ivRatio=${ivRatioPeriod}, momentum=${momentumPeriod})`
            )
          ...
          const maxPossibleLongPeriod = Math.max(
            10,
            ivRatioData.length - ivRatioPeriod - momentumPeriod - 5
          )
          const longPeriod = Math.min(idealLongPeriod, maxPossibleLongPeriod)

          console.log(
            `📊 ${symbol}: ivRatioData=${ivRatioData.length} points, using longPeriod=${longPeriod} (ideal=${idealLongPeriod})`
          )

          const normalizedIV = this.calculateNormalizedIVRatio(
            ivRatioData,
            ivRatioPeriod,
            longPeriod
          )
          ...
          if (ivMomentumData.length === 0) {
            console.warn(
              `⚠️ No momentum data for ${symbol}: normalizedIV has ${normalizedIV.length} points, need ${momentumPeriod + 1} minimum`
            )
            console.warn(
              `   This usually means insufficient IV data from API. Try: (1) Longer timeframe, (2) Different symbol with more options liquidity`
            )
            continue
          }
          ...
          console.log(
            `📊 ${symbol}: ivMomentumData has ${ivMomentumData.length} points, requesting ${tailLength} tail points`
          )
          ...
          console.log(
            `📊 ${symbol}: Created tail with ${tail.length} points (requested: ${actualTailLength})`
          )
          ...
          console.log(
            `✅ ${symbol}: IV-Ratio=${latest.normalizedIV.toFixed(2)}, IV-Momentum=${latest.ivMomentum.toFixed(2)}`
          )
        } catch (error) {
