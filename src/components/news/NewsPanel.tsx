    ...
    <div className="h-full flex flex-col bg-black">
      ...
      <div className="px-6 py-1 md:py-5 border-b border-gray-800 bg-black relative">
        ...
        <div className="flex items-center gap-3 mb-4">
          {/* Accent bar */}
          <div className="w-1 h-8 rounded-full bg-gradient-to-b from-orange-400 to-orange-600 shrink-0" />
          <div className="flex flex-col leading-none">
            <span className="text-[10px] font-semibold tracking-[0.25em] text-orange-400/80 uppercase mb-0.5">
              Live Feed
            </span>
          ...
          <div className="ml-1 flex items-center gap-1.5">
            ...
            <span className="text-[10px] font-medium text-orange-400/70 tracking-widest uppercase hidden md:inline">
              Live
            </span>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          >
            <TbRefresh
              className={`w-4 h-4 md:w-5 md:h-5 text-gray-400 group-hover:text-orange-400 transition-colors ${loading ? 'animate-spin' : ''}`}
            />
          </button>
        </div>
      </div>
              ...
              <div className="mb-5">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {marketSentiment.trending_topics.slice(0, 8).map((topic) => (
                    ...
                    >
                      <span className="text-xs text-white/90 font-medium">{topic.keyword}</span>
                      <span className="text-[10px] text-gray-500">{topic.mentions}</span>
                    ...
                    <div className="p-6">
                      ...
                      <div className="flex items-start justify-between mb-4">
                        <div className="flex items-center gap-3 flex-wrap">
                          ...
                          <span
                            className={`px-3 py-1.5 rounded-lg text-xs font-bold tracking-wide border ${getCategoryBadgeColor(article.category)}`}
                          >
                            {article.category.toUpperCase()}
                          </span>
                          {article.tickers.length > 0 && (
                            <div className="flex gap-2">
                              {[...new Set(article.tickers)].slice(0, 3).map((ticker) => (
                                <span
                                  key={ticker}
                                  className="px-3 py-1.5 bg-orange-500/20 text-orange-300 rounded-lg text-xs font-bold tracking-wider border border-orange-500/30"
                                >
                                  {ticker}
                                </span>
                              ))}
                              {[...new Set(article.tickers)].length > 3 && (
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3 text-xs text-gray-400">
                          ...
                          <div
                            className={`flex items-center gap-1 ${getSentimentColor(article.sentiment, article.sentiment_score)}`}
                          >
