...
export default function LeadershipScan() {
  ...
  const calculateLeadershipMetrics = async (
    symbol: string,
    customTimeframe?: number,
    customMinDaysBelow?: number
  ): Promise<LeadershipStock | null> => {
    try {
      const tf = customTimeframe ?? timeframe
      const minDays = customMinDaysBelow ?? minDaysBelow
      const endDate = new Date().toISOString().split('T')[0]
      const startDate = new Date(Date.now() - tf * 365 * 24 * 60 * 60 * 1000)
        .toISOString()
        .split('T')[0]
    ...
    <div
      ...
      onMouseEnter={(e) => {
        ...
        e.currentTarget.style.boxShadow =
          '0 8px 25px rgba(0, 0, 0, 0.4), 0 0 20px rgba(255, 140, 0, 0.3)'
      }}
      onMouseLeave={(e) => {
    ...
    >
      ...
      >
        ...
        <div>
          ...
          >
            {leader.priceChangePercent >= 0 ? '+' : ''}
            {leader.priceChangePercent.toFixed(2)}%
          </div>
        </div>
        <div>
          <div
          ...
          <div
            style={{
              color:
                leader.daysSinceLastHigh >= 60
                  ? '#00ff41'
                  : leader.daysSinceLastHigh >= 30
                    ? '#ffff00'
                    : '#ff8c00',
              fontWeight: '800',
              fontSize: '12px',
    ...
    >
      ...
      <div
        style={{
          ...
          justifyContent: 'space-between',
          position: 'relative',
          background: 'linear-gradient(135deg, #000000 0%, #1a1608 50%, #000000 100%)',
          padding: '16px 24px',
          borderRadius: '0',
          gap: '24px',
        }}
      >
        ...
        <div style={{ flex: 1 }}>
          ...
          >
            <span style={{ color: '#ff8c00' }}>LEADERSHIP</span> SCAN
          </h1>
          <div
            style={{
              color: '#ffffff',
          ...
          >
            Stocks breaking OUT to ATH/52W high after being below for{' '}
            <span style={{ color: '#ff8c00' }}>{minDaysBelow}+</span> days
            {lastUpdate && (
              <span
                style={{
                  marginLeft: '16px',
        ...
        {leaders.length > 0 ? (
          ...
          >
            ...
            <div style={{ fontSize: '12px' }}>
              Find stocks breaking OUT to reach ATH/52W high after {minDaysBelow}+ days below
            </div>
          </div>
        ) : null}
      </div>
