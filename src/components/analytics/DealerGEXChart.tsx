...
export default function DealerGEXChart({
  ...
  const [attraxPoints, setAttraxPoints] = useState<
    Array<{
      strikes: number[]
      values: number[]
      peakStrike: number
      ...
      for (let j = i + 1; j < uniqueStrikes.length && sequence.length < 5; j++) {
        ...
        if (isPositiveSequence) {
          ...
          if (prevValue >= 0) {
            ...
            isIncreasing =
              currentValue >= prevValue * 1.1 ||
              (currentValue >= 0 && Math.abs(prevValue) * 1.1 <= Math.abs(currentValue))
          }
        } else {
  ...
  useEffect(() => {
    ...
    const fetchGEXData = async () => {
      try {
        ...
        const daysMatchGEX = selectedExpiration.match(/^(\d+)-days$/)
        if (daysMatchGEX) {
          const nDays = parseInt(daysMatchGEX[1])
          const response = await fetch(`/api/dealer-options-premium?ticker=${selectedTicker}`)
          const result = await response.json()
    ...
    const zoom = d3
      ...
      .on('zoom', (event) => {
        ...
        const minVisibleValue =
          viewMode === 'premium' ? 0 : d3.min(visibleData, (d) => d.gammaExposure) || 0

        // Add 20% padding to negative values so bars don't touch the X-axis
        const adjustedMinVisibleValue =
          minVisibleValue < 0 ? minVisibleValue * 1.2 : minVisibleValue
        ...
        if (viewMode === 'gex') {
          container.select('.zero-line').attr('y1', newYScale(0)).attr('y2', newYScale(0))
        }
        ...
        if (propShowAttrax && showNetGamma && attraxPoints.length > 0) {
          attraxPoints.forEach((pattern, patternIndex) => {
            ...
            if (!allStrikesVisible) {
              overlayLayer
                .selectAll(`.attrax-curve-${patternIndex}, .attrax-icon-${patternIndex}`)
                .style('display', 'none')
              return
            }
            ...
            if (pathData) {
              overlayLayer.select(`.attrax-curve-${patternIndex}`).attr('d', pathData)
              ...
              overlayLayer
                .select(`.attrax-icon-${patternIndex}`)
    ...
    container
      ...
      .on('mouseover', function (event, d) {
        ...
        const tooltip = container
          ...
          .attr(
            'transform',
            `translate(${(xScale(d.strike.toString()) || 0) + xScale.bandwidth() / 2}, ${yScale(yValue(d)) - 10})`
          )
    ...
    if (propShowAttrax && attraxPoints.length > 0 && showNetGamma) {
      attraxPoints.forEach((pattern, patternIndex) => {
        ...
        if (pathData) {
          ...
          gradient.append('stop').attr('offset', '0%').attr('stop-color', '#ff8800')

          gradient.append('stop').attr('offset', '100%').attr('stop-color', '#0088ff')

          iconGroup.append('circle').attr('r', 5).attr('fill', `url(#${gradientId})`)
