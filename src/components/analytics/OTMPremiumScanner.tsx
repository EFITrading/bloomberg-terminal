...
export default function OTMPremiumScanner({ compactMode = false }: OTMPremiumScannerProps) {
    ...
    // Fetch available expiration dates from AAPL
    const fetchExpirationDates = async () => {
        setLoadingExpiries(true);
        try {
            const response = await fetch('/api/dealer-options-premium?ticker=AAPL');
            ...
            if (result.success && result.data) {
                ...
                // Set default to first available date or monthly expiry
                if (dates.length > 0) {
                    setOtmExpiry(dates[0]);
                } else {
            ...
            <div className="bg-gradient-to-r from-slate-950 via-red-950/10 to-green-950/10 border-b border-gray-800 px-6 py-4">
                <div className="flex flex-wrap items-center gap-4">
                    ...
                    <div className="flex items-center gap-3 px-4 py-3 bg-slate-900 border border-gray-800 rounded-lg">
                        ...
                        <select
                            value={otmExpiry}
                            onChange={(e) => setOtmExpiry(e.target.value)}
                            disabled={loadingExpiries || otmLoading}
                            className="bg-slate-800 border-none text-base font-medium text-white focus:outline-none disabled:opacity-50"
                        >
                            {loadingExpiries ? (
                                ...
                                <option value={otmExpiry}>{formatExpiryDate(otmExpiry)}</option>
                            )}
                        </select>
                    </div>
