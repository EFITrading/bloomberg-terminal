...
const MarketHeatmap: React.FC = () => {
  ...
  const stockSectorMap: { [key: string]: string } = {
    ...
    STLD: 'Materials',
    CLF: 'Materials',
    X: 'Non-Energy Minerals',
    MP: 'Non-Energy Minerals',
    AA: 'Non-Energy Minerals',
  }
  ...
  const stockIndustryMap: { [key: string]: string } = {
    ...
    CVS: 'Medical Distribution',
    CI: 'Healthcare Plans',
    HUM: 'Healthcare Plans',
    MDT: 'Medical Devices',
    SYK: 'Medical Devices',
    BDX: 'Medical Devices',
    ZTS: 'Drug Manufacturers - General',
    BIIB: 'Biotech',
    IDXX: 'Diagnostics',
    ...
    IQV: 'Diagnostics',
    ILMN: 'Diagnostics',
    AZN: 'Drug Manufacturers - General',
    WBA: 'Medical Distribution',
    ...
    PG: 'Household',
    KO: 'Beverages - Household',
    PEP: 'Beverages - Household',
    WMT: 'Discount Stores',
    ...
    EOG: 'Oil & Gas E&P',
    SLB: 'Oil & Gas E&P',
    PSX: 'Oil & Gas Refining',
    VLO: 'Oil & Gas Refining',
    MPC: 'Oil & Gas Refining',
    OXY: 'Oil & Gas E&P',
    BKR: 'Oil & Gas E&P',
    HAL: 'Oil & Gas E&P',
    DVN: 'Oil & Gas E&P',
    ...
    MMM: 'Conglom',
    DE: 'Farm & Heavy',
    FDX: 'Integrated Freight',
    NOC: 'Aerospace & Defense',
    WM: 'Waste Management',
    EMR: 'Specialty Industrial',
    ETN: 'Specialty Industrial',
    ITW: 'Specialty Industrial',
    CSX: 'Railroads',
    NSC: 'Railroads',
    CARR: 'Building',
    PCAR: 'Farm & Heavy',
    ...
    CHTR: 'Entertainment',
    EA: 'Consumer Cyclical',
    TTWO: 'Consumer Cyclical',
    MTCH: 'Internet Content',
    ROKU: 'Consumer Cyclical',
    ...
    NEE: 'Utilities - Regulated',
    SO: 'Utilities - Regulated',
    DUK: 'Utilities - Regulated',
    CEG: 'Utilities - Independent',
    SRE: 'Utilities - Regulated',
    AEP: 'Utilities - Regulated',
    VST: 'Utilities - Independent',
    D: 'Utilities - Regulated',
    PCG: 'Utilities - Regulated',
    PEG: 'Utilities - Regulated',
    EXC: 'Utilities - Regulated',
    XEL: 'Utilities - Regulated',
    EIX: 'Utilities - Regulated',
    WEC: 'Utilities - Regulated',
    AWK: 'Utilities - Regulated',
    DTE: 'Utilities - Regulated',
    PPL: 'Utilities - Regulated',
    ES: 'Utilities - Regulated',
    AEE: 'Utilities - Regulated',
    CMS: 'Utilities - Regulated',
  ...
  const loadHeatmapData = async () => {
    ...
    try {
          ...
          try {
            ...
            if (data && data.ticker) {
              ...
              if (timePeriod === 'today') {
                ...
                changePercent =
                  yearAgoPrice !== 0 ? ((currentPrice - yearAgoPrice) / yearAgoPrice) * 100 : 0
              }
      ...
      if (stocks.length > 0) {
        ...
        const sectorsData: SectorData[] = Object.entries(sectorGroups).map(
  ...
  const getBoxSize = (
    ...
    const minMarketCap = Math.min(
      ...allStocks.filter((s) => s.marketCap > 0).map((s) => s.marketCap)
    )

    const relativeSize =
      maxMarketCap > 0 && stock.marketCap > 0
  ...
  const getMarketStats = () => {
    ...
    const maxGainer = stocks.reduce(
      (max, s) => (s.changePercent > max.changePercent ? s : max),
      stocks[0]
    )
    const maxLoser = stocks.reduce(
      (min, s) => (s.changePercent < min.changePercent ? s : min),
      stocks[0]
    )
  ...
  const renderHeatmapGrid = () => {
            ...
            return (
              ...
              >
                ...
                <div
                  ...
                  style={{ color: stock.changePercent >= 0 ? '#00ff00' : '#ff0000' }}
                >
    ...
    return (
      <div
        className="heatmap-treemap-container"
        style={{ width, height, position: 'relative', background: '#000' }}
      >
        {filteredSector && (
          <div
            onClick={() => {
              ...
              >
                ...
                <div
                  onDoubleClick={() => {
                    if (!filteredSector) {
                      const sectorStocks =
                        heatmapData.find((s) => s.name === sectorNode.data.name)?.stocks || []
                      ...
                      return (
                        ...
                        >
                          ...
                          {showChange && (
                            ...
                            >
                              {stockNode.data.changePercent >= 0 && '+'}
                              {stockNode.data.changePercent.toFixed(1)}%
                            </div>
                          )}
                        </div>
                      )
  ...
  useEffect(() => {
    ...
    const log = () => {
      ...
      console.log(`[HEATMAP] ${Math.round(r.width)}×${Math.round(r.height)}px top=${Math.round(r.top)} scrollH=${el.scrollHeight} win=${window.innerWidth}×${window.innerHeight} |${chain}`)
    }
    log()
  ...
  return (
    <div ref={_dbgRef} className="market-heatmap-container">
      <div
          ...
          >
            ...
            {selectedView === 'sectors'
              ...
              : selectedView === 'industries'
                ? 'Industry'
                : 'Index'}{' '}
            and click SCAN to load heatmap
          </div>
        </div>
      )}
      ...
      {loading ? (
        ...
        <div className="heatmap-content">{renderHeatmapGrid()}</div>
      ) : null}
    </div>
  )
}
