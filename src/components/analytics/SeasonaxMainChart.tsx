...
const smoothData = (data: DailySeasonalData[]): DailySeasonalData[] => {
  ...
  const smoothed = data.map((point, index) => {
    ...
    const smoothedReturn =
      (prev.cumulativeReturn + point.cumulativeReturn + next.cumulativeReturn) / 3
...
const drawSeasonalLine = (
  ...
  dataPoints.forEach((dayData, index) => {
    ...
    const y =
      containerHeight -
      padding.bottom -
      ((dayData.cumulativeReturn - paddedMin) / paddedRange) * chartHeight
...
const SeasonaxMainChart: React.FC<SeasonaxMainChartProps> = ({
  ...
  const drawMainSeasonalChart = () => {
    ...
    // Get full container size — use offsetWidth/offsetHeight to get CSS pixel
    // dimensions unaffected by any CSS zoom applied on ancestor elements.
    // getBoundingClientRect() returns the *visual* (zoomed) size, causing the
    // canvas to draw at zoom² of the container and leaving gray space on the right.
    ...
    try {
      ...
      let allCumulativeReturns =
        visibleData.length > 0
          ? visibleData.map((d) => d.cumulativeReturn)
          : processedData.map((d) => d.cumulativeReturn)
      ...
      comparisonData.forEach((compData) => {
        if (compData && compData.dailyData) {
          ...
          const compReturns =
            visibleCompData.length > 0
              ? visibleCompData.map((d) => d.cumulativeReturn)
              : compProcessedData.map((d) => d.cumulativeReturn)
      ...
      if (compareData && compareData.dailyData) {
        ...
        const compareReturns =
          visibleCompareData.length > 0
            ? visibleCompareData.map((d) => d.cumulativeReturn)
            : compareProcessedData.map((d) => d.cumulativeReturn)
      ...
      if (processedData && processedData.length > 0) {
        ...
        for (let i = 0; i < processedData.length - 1; i++) {
          ...
          const y1 =
            containerHeight -
            padding.bottom -
            ((currentDay.cumulativeReturn - paddedMin) / paddedRange) * chartHeight
          const x2 = getXPosition(nextDay)
          const y2 =
            containerHeight -
            padding.bottom -
            ((nextDay.cumulativeReturn - paddedMin) / paddedRange) * chartHeight
      ...
      if (
        ...
        currentYearSeries.forEach((point, idx) => {
          ...
          const y =
            containerHeight -
            padding.bottom -
            ((point.cumulativeReturn - paddedMin) / paddedRange) * chartHeight
        ...
        if (labelX >= padding.left && labelX <= containerWidth - padding.right) {
          ...
          ctx.fillText(
            ...
            Math.min(labelX + 4, containerWidth - padding.right - 30),
            labelY
          )
        }
      ...
      if (mousePos) {
        ...
        if (
          ...
          mouseY >= padding.top &&
          mouseY <= containerHeight - padding.bottom
        ) {
          // Draw vertical line
