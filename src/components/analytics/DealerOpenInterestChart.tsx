...
export default function DealerOpenInterestChart({
  ...
  const [towerStructures, setTowerStructures] = useState<
    Array<{ strike: number; leftStrike: number; rightStrike: number; type: 'call' | 'put' }>
  >([])
    ...
    typesToCheck.forEach((type) => {
      for (let i = 0; i < uniqueStrikes.length; i++) {
        ...
        for (const distance of distancesToCheck) {
          ...
          if (leftPercent >= 25 && leftPercent <= 65 && rightPercent >= 25 && rightPercent <= 65) {
            // Check if this tower is already detected (avoid duplicates)
            const alreadyExists = towers.some((t) => t.strike === centerStrike && t.type === type)
  ...
  useEffect(() => {
    ...
    const fetchOptionsData = async () => {
      try {
        ...
        // Handle "N-days" aggregation (e.g. '7-days', '30-days', '45-days')
        const daysMatchOI = selectedExpiration.match(/^(\d+)-days$/)
        if (daysMatchOI) {
          const nDays = parseInt(daysMatchOI[1])
          const response = await fetch(`/api/dealer-options-premium?ticker=${selectedTicker}`)
          const result = await response.json()

          if (result.success && result.data) {
            const today = new Date()
            const maxDate = new Date(today.getTime() + nDays * 24 * 60 * 60 * 1000)

            // Filter expirations within the selected day window
            const validExpirations = Object.keys(result.data).filter((exp) => {
              const expDate = new Date(exp + 'T16:00:00')
            ...
            if (validExpirations.length === 0) {
              setError(`No expirations within ${nDays} days`)
              setLoading(false)
              return
            }
        ...
        const response = await fetch(
          `/api/dealer-options-premium?ticker=${selectedTicker}&expiration=${selectedExpiration}`
        )
        const result = await response.json()
        ...
        if (result.success && result.data && result.data[selectedExpiration]) {
          ...
          const strikeMap = new Map<
            number,
            { callOI: number; putOI: number; callPremium: number; putPremium: number }
          >()
  ...
  useEffect(() => {
    ...
    const calculateExpectedRange = async () => {
      try {
        // Handle N-days aggregation (e.g. '7-days', '30-days', '45-days')
        const daysMatchER = selectedExpiration.match(/^(\d+)-days$/)
        if (daysMatchER) {
          const nDays = parseInt(daysMatchER[1])
          const response = await fetch(`/api/dealer-options-premium?ticker=${selectedTicker}`)
          const result = await response.json()
          ...
          // Get all expirations within the selected day window
          const validExpirations = Object.keys(result.data).filter((exp) => {
            const expDate = new Date(exp + 'T16:00:00')
            return expDate >= today && expDate <= maxDate
          })
          ...
          if (totalCallOI === 0 && totalPutOI === 0) {
            ...
            setExpectedRangePCRatio(
              `${ratio.toFixed(2)} (${put90.toFixed(0)}-${call90.toFixed(0)})`
            )
          }
    ...
    const calculate45DayPCRatio = async () => {
      try {
        ...
        for (const exp of expirations45Days) {
          try {
            const response = await fetch(
              `/api/dealer-options-premium?ticker=${selectedTicker}&expiration=${exp}`
            )
            const result = await response.json()
  ...
  useEffect(() => {
    ...
    const zoom = d3
      ...
      .on('zoom', (event) => {
        ...
        const newYScale = d3.scaleLinear().domain([0, maxVisibleValue]).range([height, 0])

        // Create new band scale for visible strikes only
        const newXBandScale = d3
          .scaleBand()
          .domain(visibleStrikes.map((s) => s.toString()))
          .range([0, width])
          .padding(0.2)

        const newXSubScale = d3
          .scaleBand()
          .domain(['call', 'put'])
        ...
        const filteredVisibleTicks = visibleStrikes.filter(
          (_, index) => index % visibleTickInterval === 0
        )

        const customVisibleXAxis = d3
          .axisBottom(newXBandScale)
          .tickValues(filteredVisibleTicks.map((s) => s.toString()))

        const xAxisUpdate = container.select('.x-axis') as d3.Selection<
          SVGGElement,
          unknown,
          null,
          undefined
        ...
        const yAxisUpdate = container.select('.y-axis') as d3.Selection<
          SVGGElement,
          unknown,
          null,
          undefined
        ...
        if (currentPrice > 0) {
          ...
          if (currentPriceX === -1) {
            ...
            const lowerX =
              (newXBandScale(lowerStrike.toString()) || 0) + newXBandScale.bandwidth() / 2
            const upperX =
              (newXBandScale(upperStrike.toString()) || 0) + newXBandScale.bandwidth() / 2
            xPosition = lowerX + ratio * (upperX - lowerX)
          }

          container.select('.current-price-line').attr('x1', xPosition).attr('x2', xPosition)

          container.select('.current-price-label').attr('x', xPosition)
        }
        ...
        if (expectedRange80) {
          ...
          container
            .select('.expected-range-put-80')
            .attr('x1', putXPosition)
            .attr('x2', putXPosition)

          container.select('.expected-range-put-label').attr('x', putXPosition)
          ...
          container
            .select('.expected-range-call-80')
            .attr('x1', callXPosition)
            .attr('x2', callXPosition)

          container.select('.expected-range-call-label').attr('x', callXPosition)
        }
        ...
        if (showTowers && towerStructures.length > 0) {
          towerStructures.forEach((tower, index) => {
            ...
            const centerBarX =
              (newXBandScale(centerStrike.toString()) || 0) + (newXSubScale(tower.type) || 0)
            const leftBarX =
              (newXBandScale(leftStrike.toString()) || 0) + (newXSubScale(tower.type) || 0)
            const rightBarX =
              (newXBandScale(rightStrike.toString()) || 0) + (newXSubScale(tower.type) || 0)
            ...
            const centerData = visibleData.find(
              (d) => d.strike === centerStrike && d.type === tower.type
            )
            const leftData = visibleData.find(
              (d) => d.strike === leftStrike && d.type === tower.type
            )
            const rightData = visibleData.find(
              (d) => d.strike === rightStrike && d.type === tower.type
            )

            const centerOI = centerData
              ? viewMode === 'premium'
                ? centerData.premium
                : centerData.openInterest
              : 0
            const leftOI = leftData
              ? viewMode === 'premium'
                ? leftData.premium
                : leftData.openInterest
              : 0
            const rightOI = rightData
              ? viewMode === 'premium'
                ? rightData.premium
                : rightData.openInterest
              : 0
            ...
            container
              .select(`.tower-${index} .tower-bridge-left`)
            ...
            container
              .select(`.tower-${index} .tower-bridge-right`)
    ...
    container
      ...
      .on('mouseover', function (event, d) {
        ...
        const tooltip = container
          ...
          .attr(
            'transform',
            `translate(${xScale(d.strike.toString())! + xScale.bandwidth() / 2}, ${yScale(yValue(d)) - 10})`
          )
    ...
    const xAxis = d3.axisBottom(xScale).tickValues(filteredTicks.map((s) => s.toString()))
    ...
    if (expectedRange80) {
      ...
      if (callStrikeIndex !== -1) {
        ...
        if (call80 === strikes[callStrikeIndex]) {
          callXPosition =
            (xScale(strikes[callStrikeIndex].toString()) || 0) + xScale.bandwidth() / 2
        } else {
