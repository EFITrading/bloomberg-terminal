...
export default function ScreenersPanel() {
    ...
    try {
      ...
      try {
        ...
        await new Promise<void>((resolve) => {
          const eventSource = new EventSource(
            `/api/scan-premium-stream?symbols=${encodeURIComponent(symbol)}&expiry=${encodeURIComponent(nextMonthExpiry)}`
          )
      ...
      try {
        const response = await fetch(
          `/api/gex-screener?symbols=${encodeURIComponent(symbol)}&expirationFilter=Default`
        )
        if (response.ok) {
      ...
      const rsPromises = lookbackPeriods.map(async (years, idx) => {
        try {
          ...
          if (
            symbolData &&
            symbolData.results &&
            symbolData.results.length > 0 &&
            spyData &&
            spyData.results &&
            spyData.results.length > 0
          ) {
            const symbolReturn =
              ((symbolData.results[symbolData.results.length - 1].c - symbolData.results[0].c) /
                symbolData.results[0].c) *
              100
            const spyReturn =
              ((spyData.results[spyData.results.length - 1].c - spyData.results[0].c) /
                spyData.results[0].c) *
              100
            ...
            const allReturns = await Promise.all(
              lookbackPeriods.map(async (y) => {
                const sd = new Date(Date.now() - y * 365.25 * 24 * 60 * 60 * 1000)
                  .toISOString()
                  .split('T')[0]
                ...
                if (
                  ...
                  spData &&
                  spData.results &&
                  spData.results.length > 0
                ) {
                  const sRet =
                    ((sData.results[sData.results.length - 1].c - sData.results[0].c) /
                      sData.results[0].c) *
                    100
                  const spRet =
                    ((spData.results[spData.results.length - 1].c - spData.results[0].c) /
                      spData.results[0].c) *
                    100
                  return sRet - spRet
                }
            ...
            if (validReturns.length > 0) {
              ...
              const percentile =
                (validReturns.filter((r) => r <= outperformance).length / validReturns.length) * 100
      ...
      const hvPromises = [
        ...
        try {
          const endDate = new Date().toISOString().split('T')[0]
          const startDate = new Date(Date.now() - (lookback + days + 30) * 24 * 60 * 60 * 1000)
            .toISOString()
            .split('T')[0]
          ...
          eventSource.onmessage = (event) => {
            try {
              ...
              if (data.type === 'result' && data.result) {
                ...
                console.log(
                  '[Unified] Pivot result added:',
                  data.result.symbol,
                  data.result.period,
                  'qualifies:',
                  data.result.qualifies
                )
              } else if (data.type === 'complete') {
                    ...
                    {searchResults.otmResults.map((result, idx) => (
                      <div
                        ...
                        style={{
                          ...
                          paddingBottom: '12px',
                          borderBottom:
                            idx < searchResults.otmResults.length - 1 ? '1px solid #333' : 'none',
                        }}
                      >
                        <div
                          style={{
                        ...
                        <div style={{ fontSize: '16px', color: '#ffffff', lineHeight: '1.5' }}>
                          <div>
                            ...
                            <span
                              style={{
                                color:
                                  result.imbalancePercent >= 75
                                    ? '#ff073a'
                                    : result.imbalancePercent >= 50
                                      ? '#ff6b00'
                                      : '#ffd700',
                              }}
                            >
                              {result.imbalancePercent.toFixed(1)}%
                            </span>{' '}
                          ...
                          <div>
                            ...
                            <span
                              style={{
                                color: result.expensiveSide === 'CALLS' ? '#00ff88' : '#ff073a',
                              }}
                            >
                              {result.expensiveSide}
                            </span>
                          </div>
                          ...
                          >
                            <div>
                              Call ${result.callStrike}: ${result.callMid.toFixed(2)} (spread:{' '}
                              {result.callSpreadPercent.toFixed(1)}%)
                            </div>
                            <div>
                              Put ${result.putStrike}: ${result.putMid.toFixed(2)} (spread:{' '}
                              {result.putSpreadPercent.toFixed(1)}%)
                            </div>
                          </div>
                        </div>
                      </div>
                    ...
                    {searchResults.attractionResults.map((result, idx) => (
                      <div
                        ...
                        style={{
                          ...
                          paddingBottom: '12px',
                          borderBottom:
                            idx < searchResults.attractionResults.length - 1
                              ? '1px solid #333'
                              : 'none',
                        }}
                      >
                        ...
                        <div style={{ fontSize: '16px', color: '#ffffff', lineHeight: '1.8' }}>
                          <div style={{ marginBottom: '4px' }}>
                            ...
                            <span
                              style={{
                                color:
                                  result.attractionLevel >= 7
                                    ? '#ff073a'
                                    : result.attractionLevel >= 5
                                      ? '#ff6b00'
                                      : '#ffd700',
                              }}
                            >
                              {result.attractionLevel.toFixed(2)}
                            </span>
                          </div>
                          <div style={{ marginBottom: '4px' }}>
                            Dealer Sweat:{' '}
                            <span
                              style={{
                                color:
                                  result.dealerSweat >= 7
                                    ? '#ff073a'
                                    : result.dealerSweat >= 5
                                      ? '#ff6b00'
                                      : '#00ff88',
                              }}
                            >
                              {result.dealerSweat.toFixed(2)}
                            </span>
                          </div>
                          <div style={{ marginBottom: '4px' }}>
                            Net GEX: {result.netGex >= 0 ? '+' : ''}
                            {(result.netGex / 1e6).toFixed(2)}M
                          </div>
                          <div style={{ marginBottom: '4px' }}>
                            Bias:{' '}
                            <span
                              style={{ color: result.bias === 'Bullish' ? '#00ff88' : '#ff073a' }}
                            >
                              {result.bias}
                            </span>
                          </div>
                          ...
                          {result.largestWall && (
                            ...
                            >
                              <div style={{ marginBottom: '4px' }}>
                                Largest Wall: ${result.largestWall.strike} (
                                {result.largestWall.type})
                              </div>
                              <div style={{ marginBottom: '4px' }}>
                                Wall GEX: {(result.largestWall.gex / 1e6).toFixed(2)}M
                              </div>
                    ...
                    {searchResults.pivotResults.map((result, idx) => (
                      <div
                        ...
                        style={{
                          ...
                          paddingBottom: '12px',
                          borderBottom:
                            idx < searchResults.pivotResults.length - 1 ? '1px solid #333' : 'none',
                        }}
                      >
                        <div
                          style={{
                        ...
                        <div style={{ fontSize: '16px', color: '#ffffff', lineHeight: '1.5' }}>
                          <div>
                            Status:{' '}
                            <span style={{ color: result.qualifies ? '#00ff88' : '#ff073a' }}>
                          ...
                          <div>
                            ...
                            <span
                              style={{
                                color:
                                  result.contractionPercent >= 40
                                    ? '#00ff88'
                                    : result.contractionPercent >= 30
                                      ? '#ffd700'
                                      : '#ffffff',
                              }}
                            >
                              {result.contractionPercent.toFixed(1)}%
                            </span>
                          </div>
                          <div>
                            Contraction Level:{' '}
                            <span
                              style={{
                                color:
                                  result.contractionLevel === 'EXTREME'
                                    ? '#ff073a'
                                    : result.contractionLevel === 'HIGH'
                                      ? '#ff6b00'
                                      : '#ffd700',
                              }}
                            >
                              {result.contractionLevel}
                            </span>
                          </div>
                          <div>
                            Squeeze:{' '}
                            <span
                              style={{
                                color: result.squeezeStatus === 'ON' ? '#00ff88' : '#ffffff',
                              }}
                            >
                          ...
                          {result.isSideways !== undefined && (
                            <div style={{ marginTop: '8px', fontSize: '15px', color: '#ffffff' }}>
                              ...
                              {result.netMovePercent !== undefined && (
                                <div>Net Move: {result.netMovePercent.toFixed(1)}%</div>
                              )}
                              {result.isAtExtremes !== undefined && (
                                <div>At Extremes: {result.isAtExtremes ? 'YES' : 'NO'}</div>
                              )}
                            </div>
                          )}
                        </div>
                ...
                {searchResults.leadershipResults.isLeader && (
                  ...
                  >
                    ...
                    <div>
                      ...
                      >
                        ✓ LEADERSHIP DETECTED (45+ day consolidation breakout)
                      </div>
                    ...
                    <div>
                      ...
                      {searchResults.liquidationResults.mli && (
                        <div style={{ fontSize: '16px', color: '#ffffff', marginLeft: '12px' }}>
                          Mass Liquidation Index (MLI):{' '}
                          <span
                            style={{
                              fontWeight: '700',
                              color:
                                searchResults.liquidationResults.mli >= 5
                                  ? '#ff073a'
                                  : searchResults.liquidationResults.mli >= 3
                                    ? '#ff6b00'
                                    : '#ffd700',
                            }}
                          >
                            {searchResults.liquidationResults.mli.toFixed(2)}
                          </span>
                        </div>
                      )}
                    </div>
  ...
  useEffect(() => {
    ...
    const log = () => {
      ...
      console.log(`[SCREENERS] ${Math.round(r.width)}×${Math.round(r.height)}px top=${Math.round(r.top)} scrollH=${el.scrollHeight} win=${window.innerWidth}×${window.innerHeight} |${chain}`)
    }
    log()
  ...
  return (
    ...
    >
      {/* Unified Search Bar */}
      <div
        className="screeners-search-bar"
        style={{
      ...
      >
        ...
        <input
          ...
          onFocus={(e) => {
            ...
            e.currentTarget.style.boxShadow =
              'inset 0 2px 4px rgba(0, 0, 0, 0.6), 0 0 12px rgba(0, 212, 255, 0.3)'
          }}
          onBlur={(e) => {
        ...
        <button
          ...
          style={{
            ...
            textTransform: 'uppercase',
            letterSpacing: '1px',
            boxShadow: isSearching ? 'none' : '0 4px 12px rgba(0, 212, 255, 0.4)',
            transition: 'all 0.3s ease',
