...
const RRGScreener: React.FC<RRGScreenerProps> = ({
  hideTitle = false,
  sectorUnderTicker = false,
  compactLayout = false,
}) => {
  ...
  const [chartTimeframe, setChartTimeframe] = useState<'1D' | '5D' | '1M' | '3M' | '6M' | '1Y'>(
    '1D'
  )
  const [sparklineData, setSparklineData] = useState<
    Record<string, Array<{ time: number; price: number }>>
  >({})
  ...
  const [quadrantFilter, setQuadrantFilter] = useState<
    'all' | 'leading' | 'lagging' | 'weakening' | 'improving' | 'consistent-3' | 'consistent-4'
  >('all')
  ...
  const getQuadrant = (
    rsRatio: number,
    rsMomentum: number
  ): 'leading' | 'lagging' | 'weakening' | 'improving' => {
  ...
  const runScreener = useCallback(async () => {
    ...
    try {
      ...
      console.log(
        `🚀 Starting SERVER-SIDE RRG scan: ${ALL_STOCKS.length} stocks in ${totalBatches} batches`
      )
      ...
      for (let i = 0; i < ALL_STOCKS.length; i += BATCH_SIZE * PARALLEL_BATCHES) {
        ...
        for (const result of batchResults) {
          ...
          if (data.success && data.results) {
            ...
            console.log(
              `✓ Batch ${result.batchIndex + 1}/${totalBatches} complete - ${results.length}/${ALL_STOCKS.length} stocks`
            )
          }
        }

        setProgress({
          current: Math.min(i + BATCH_SIZE * PARALLEL_BATCHES, ALL_STOCKS.length),
      ...
      console.log(
        `💡 SMART: Sparkline data already cached - 5D/1M/3M/6M/1Y require ZERO extra API calls!`
      )
    } catch (err) {
                ...
                if (spyData.length > 0 && stockData.length > 0) {
                  ...
                  const relativeData = stockData.map((point: { time: number; price: number }) => {
                    ...
                    const spyPoint = spyData.reduce((prev, curr) =>
                      Math.abs(curr.time - point.time) < Math.abs(prev.time - point.time)
                        ? curr
                        : prev
                    )
        ...
        <div style={styles.headerTop}>
          ...
          <div style={styles.controlGroup}>
            ...
            <select
              ...
              disabled={loading}
            >
              <option value="all" style={{ background: '#0a0f1a', color: '#fff' }}>
                ALL ({data.length})
              </option>
              <option value="leading" style={{ background: '#0a0f1a', color: '#fff' }}>
                LEADING ({counts.leading})
              </option>
              <option value="weakening" style={{ background: '#0a0f1a', color: '#fff' }}>
                WEAKENING ({counts.weakening})
              </option>
              <option value="lagging" style={{ background: '#0a0f1a', color: '#fff' }}>
                LAGGING ({counts.lagging})
              </option>
              <option value="improving" style={{ background: '#0a0f1a', color: '#fff' }}>
          ...
          <div style={styles.controlGroup}>
            ...
            <select
              ...
              disabled={loading}
            >
              <option value="consistency" style={{ background: '#0a0f1a', color: '#fff' }}>
                CONSISTENCY
              </option>
              <option value="consistent-4" style={{ background: '#0a0f1a', color: '#fff' }}>
                4/4 CONSISTENT
              </option>
              <option value="consistent-3" style={{ background: '#0a0f1a', color: '#fff' }}>
                3+/4 CONSISTENT
              </option>
              <option value="slingshot" style={{ background: '#0a0f1a', color: '#fff' }}>
                SLINGSHOT
              </option>
              <option value="rsRatio" style={{ background: '#0a0f1a', color: '#fff' }}>
                RS RATIO
              </option>
              <option value="rsMomentum" style={{ background: '#0a0f1a', color: '#fff' }}>
                RS MOMENTUM
              </option>
              <option value="symbol" style={{ background: '#0a0f1a', color: '#fff' }}>
                SYMBOL
              </option>
            </select>
          </div>
            ...
            onMouseEnter={(e) => {
              if (!loading) {
                ...
                e.currentTarget.style.boxShadow =
                  'inset 0 1px 0 rgba(255, 255, 255, 0.3), 0 4px 12px rgba(0, 0, 0, 0.6)'
              }
            }}
            onMouseLeave={(e) => {
              e.currentTarget.style.transform = 'translateY(0)'
              e.currentTarget.style.boxShadow =
                'inset 0 1px 0 rgba(255, 255, 255, 0.25), 0 2px 8px rgba(0, 0, 0, 0.5)'
            }}
          >
        ...
        <div style={styles.quadrantSummary}>
          <div
            ...
            onMouseEnter={(e) => {
              ...
              e.currentTarget.style.boxShadow =
                '6px 6px 12px rgba(0, 0, 0, 0.9), -3px -3px 10px rgba(30, 30, 30, 0.4), inset 0 1px 0 rgba(255, 255, 255, 0.05)'
            }}
            onMouseLeave={(e) => {
              e.currentTarget.style.transform = 'translateY(0)'
              e.currentTarget.style.boxShadow =
                '5px 5px 10px rgba(0, 0, 0, 0.8), -2px -2px 8px rgba(20, 20, 20, 0.3), inset 0 1px 0 rgba(255, 255, 255, 0.03)'
            }}
          >
            <div style={{ ...styles.quadrantLabel, color: '#ffffff' }}>4/4 CONSISTENT</div>
            <div style={{ ...styles.quadrantCount, color: '#ffffff' }}>
              {data.filter((d) => d.consistency === 4).length}
            </div>
          </div>
          <div
            ...
            onMouseEnter={(e) => {
              ...
              e.currentTarget.style.boxShadow =
                '5px 5px 10px rgba(0, 0, 0, 0.9), -2px -2px 8px rgba(30, 30, 30, 0.4), inset 0 1px 0 rgba(255, 255, 255, 0.05)'
            }}
            onMouseLeave={(e) => {
              e.currentTarget.style.transform = 'translateY(0)'
              e.currentTarget.style.boxShadow =
                '4px 4px 8px rgba(0, 0, 0, 0.8), -2px -2px 6px rgba(20, 20, 20, 0.3), inset 0 1px 0 rgba(255, 255, 255, 0.03)'
            }}
          >
                  ...
                  <td style={styles.tableCell}>
                    {sectorUnderTicker ? (
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '2px' }}>
                        ...
                        <span style={{ ...styles.sector, fontSize: '11px', color: '#888' }}>
                          {item.sector}
                        </span>
                      </div>
                    ) : (
                        ...
                        >
                          {(() => {
                            ...
                            if (chartTimeframe === '1D' && data.length >= 2) {
                              // Convert to PST time with AM/PM
                              ...
                              const lastET = new Date(data[data.length - 1].time).toLocaleString(
                                'en-US',
                                {
                                  timeZone: 'America/Los_Angeles',
                                  hour: 'numeric',
                              ...
                              // For multi-day charts, show dates in PST
                              const first = new Date(data[0].time).toLocaleDateString('en-US', {
                                timeZone: 'America/Los_Angeles',
                                month: 'short',
                                day: 'numeric',
                              })
                              const last = new Date(data[data.length - 1].time).toLocaleDateString(
                                'en-US',
                                {
                                  timeZone: 'America/Los_Angeles',
                                  month: 'short',
...
const styles: { [key: string]: React.CSSProperties } = {
  ...
  header: {
    ...
    boxShadow:
      '4px 4px 12px rgba(0, 0, 0, 0.9), -2px -2px 8px rgba(25, 25, 25, 0.2), inset 0 1px 0 rgba(255, 255, 255, 0.02)',
  },
  headerTop: {
    display: 'flex',
    justifyContent: 'space-between',
  ...
  scanButton: {
    ...
    boxShadow:
      '3px 3px 8px rgba(0, 0, 0, 0.8), -1px -1px 4px rgba(255, 149, 0, 0.2), inset 0 1px 0 rgba(255, 255, 255, 0.3)',
    textTransform: 'uppercase',
  },
  controls: {
    display: 'flex',
  ...
  select: {
    ...
    cursor: 'pointer',
    boxShadow:
      '2px 2px 6px rgba(0, 0, 0, 0.8), -1px -1px 3px rgba(30, 30, 30, 0.3), inset 0 1px 0 rgba(255, 255, 255, 0.02)',
    transition: 'all 0.2s ease',
    fontWeight: 600,
    minWidth: '160px',
  },
