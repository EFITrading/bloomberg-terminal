...
export default function TradingViewChart({
  ...
  const handleLiveFlowMovesClick = async (timeframe: '1D' | '3D' | '1W' = '1D') => {
    console.log(`🚀 Starting Live FlowMoves scan for ${symbol} - Timeframe: ${timeframe}`);
      ...
      eventSource.onmessage = async (event) => {
        try {
          ...
          if (data.type === 'complete' && data.trades?.length > 0) {
            ...
            console.log(`📅 Trading days for ${timeframe}:`, tradingDays);
            ...
            console.log(`📊 Created ${intervalData.size} intervals for ${timeframe}`);
            ...
            console.log('✅ FlowMoves data processed:', chartData.length, 'intervals');
            console.log('🔍 DEBUG: Sample chartData timestamps:');
            ...
            if (timeframe !== '1D' && chartData.length > 0) {
              ...
              console.log('✅ Added afterhours flat line points:', finalChartData.length, 'total points');
              console.log('📊 First 10 points:', finalChartData.slice(0, 10).map(p => ({ time: new Date(p.time).toLocaleString(), label: p.timeLabel, netFlow: p.netFlow })));
              if (finalChartData.length > 0) {
                console.log('📊 First flow point:', new Date(finalChartData[0].time).toISOString(), finalChartData[0].timeLabel);
                console.log('📊 Last flow point:', new Date(finalChartData[finalChartData.length - 1].time).toISOString(), finalChartData[finalChartData.length - 1].timeLabel);
              }
              setFlowChartData(finalChartData);
            } else {
              console.log('📊 Sample data points:', chartData.slice(0, 3));
              if (chartData.length > 0) {
                console.log('📊 First flow point:', new Date(chartData[0].time).toISOString(), chartData[0].timeLabel);
                console.log('📊 Last flow point:', new Date(chartData[chartData.length - 1].time).toISOString(), chartData[chartData.length - 1].timeLabel);
              }
  ...
  useEffect(() => {
    const fetchRealMarketData = async (isInitialLoad = false) => {
      ...
      console.log('?? Loading data with worker-based batching...');
 ...
 transition-all duration-500 ease-out active:scale-95 rounded-lg overflow-hidden backdrop-blur-xl"
                      ...
                      >
                        ...
                        <span
                          className="relative text-4xl transition-all duration-500 group-hover:scale-115 group-hover:rotate-6"
                          style={{
                        ...
                        <div style={{ overflow: 'auto', height: '100%' }}>
                          ...
                          <div style={{
                            ...
                            <button
                              ...
                              {seasonalScreenerFilters.highWinRate ? '✓ ' : ''}60%+ Win Rate
                            </button>
                            ...
                            <button
                              ...
                              {seasonalScreenerFilters.fiftyTwoWeek ? '✓ ' : ''}52WK H/L
                            </button>
