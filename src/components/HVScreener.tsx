...
const HVScreener: React.FC = () => {
 ...
 console.log(`${symbol}: Insufficient data - got ${data?.results?.length || 0} bars, need at least ${days + 10}`);
 return null;
 }
 ...
 console.log(`${symbol}: No valid HV values calculated`);
 return null;
 }
 ...
 console.log(`${symbol} (${days}d): Current HV=${currentHV.toFixed(2)}%, Low=${hvLow.toFixed(2)}%, ${percentFromLow.toFixed(1)}% from low, Near Low: ${isNearLow}`);
 ...
 try {
 // Process all stocks with progress tracking
 ...
 // 10-day HV: lookback 3 months (90 days)
 // 20-day HV: lookback 1 year (365 days)
 // 52-week HV: lookback 5 years (1825 days / 252 trading days * 5 = 1260 trading days)
 ...
 console.log(`✓ ${symbol} added to 10-day HV column - ${percentFromLow.toFixed(1)}% from low`);

 setSignals(prevSignals => {
 // Remove any existing entry for this symbol before adding
 const filtered = prevSignals.hv10Day.filter(s => s.symbol !== symbol);
 ...
 .sort((a, b) => a.percentFromLow - b.percentFromLow)
 .slice(0, 30);
 return { ...prevSignals, hv10Day: newSignals };
 });
 }
 ...
 console.log(`✓ ${symbol} added to 20-day HV column - ${percentFromLow.toFixed(1)}% from low`);

 setSignals(prevSignals => {
 // Remove any existing entry for this symbol before adding
 const filtered = prevSignals.hv20Day.filter(s => s.symbol !== symbol);
 const newSignals = [...filtered, {
 symbol,
 ...
 .sort((a, b) => a.percentFromLow - b.percentFromLow)
 .slice(0, 30);
 return { ...prevSignals, hv20Day: newSignals };
 });
 }
 ...
 console.log(`✓ ${symbol} added to 52-week HV column - ${percentFromLow.toFixed(1)}% from low`);

 setSignals(prevSignals => {
 // Remove any existing entry for this symbol before adding
 const filtered = prevSignals.hv52Week.filter(s => s.symbol !== symbol);
 const newSignals = [...filtered, {
 symbol,
 ...
 .sort((a, b) => a.percentFromLow - b.percentFromLow)
 .slice(0, 30);
 return { ...prevSignals, hv52Week: newSignals };
 });
 }
 } catch (error) {
 ...
 Scanning {ALL_STOCKS.length} tickers | Low volatility compression setups
 {lastUpdate && (
 <span style={{ 
 marginLeft: '16px', 
 color: '#666666',
 ...
 border: loading ? '1px solid #2a2a2a' : '1px solid #ff8c00',
 padding: '10px 24px',
 borderRadius: '3px',
 fontSize: '20px',
 fontWeight: '700',
 ...
 boxShadow: loading ? 'none' : '0 2px 8px rgba(255, 140, 0, 0.3)',
