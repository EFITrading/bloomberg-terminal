...
const SpecialPerformanceChart: React.FC = () => {
  ...
  const fetchSpecialData = async () => {
    ...
    try {
      ...
      const fetchWithRetry = async (
        etf: (typeof SPECIAL_ETFS)[0],
        retries = 5
      ): Promise<SpecialPerformance | null> => {
        const url = `/api/polygon/v2/aggs/ticker/${etf.symbol}/range/${multiplier}/${timespan}/${from}/${to}?adjusted=true&sort=asc&apiKey=${POLYGON_API_KEY}`
      ...
      console.log(
        '🔵 Valid results sample:',
        validResults.slice(0, 2).map((r) => ({ symbol: r.symbol, dataPoints: r.data.length }))
      )
    } catch (error) {
  ...
  useEffect(() => {
    console.log(
      '🔵 Draw effect triggered, performanceData length:',
    ...
    if (crosshair) {
      ...
      const yValue =
        ((margin.top + chartHeight - crosshair.y) / chartHeight) * (yRange + 2 * yPadding) +
        (minY - yPadding)
        ...
        if (x >= margin.left && x <= margin.left + chartWidth) {
          ...
          if (timeframe === '1D') {
            ...
            const visibleTimeSpan =
              filteredData[0].data[endIdx - 1]?.timestamp -
              filteredData[0].data[startIdx]?.timestamp
  ...
  const handleMouseMove = (e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    sortedData.forEach((etf, index) => {
      ...
      if (
        ...
        // Expanded to cover ticker + percentage
