...
const SECTORS_AND_ETFS = [
  ...
  { symbol: 'XLC', name: 'Communication', color: '#ff006e' },
...
const SectorPerformanceChart: React.FC = () => {
  ...
  const fetchSectorData = async () => {
    ...
    try {
      const promises = SECTORS_AND_ETFS.map(async (sector) => {
        ...
        if (data.results && data.results.length > 0) {
          ...
          const normalizedData: SectorDataPoint[] = data.results.map((point: any) => ({
            timestamp: point.t,
  ...
  useEffect(() => {
      ...
      if (containerRef.current) {
        ...
        setDimensions(prev => {
          ...
          if (prev.width !== width || prev.height !== height) {
            console.log('📊 Chart dimensions updated:', { width, height });
            return { width, height };
          }
          return prev;
        });
    ...
    // Initial update with small delay to ensure container is mounted
  ...
  // Draw chart
  useEffect(() => {
    if (!canvasRef.current || performanceData.length === 0 || dimensions.width === 0 || dimensions.height === 0) {
      return;
    }
    ...
    const drawChart = () => {
      ...
      // Chart margins - smaller on mobile for more space, no legend space needed
      const margin = isMobile 
        ? { top: 15, right: 10, bottom: 25, left: 40 }
        : { top: 40, right: 100, bottom: 80, left: 60 };
    ...
    const gridLines = isMobile ? 3 : Math.min(10, Math.max(5, Math.floor(5 * Math.sqrt(zoom))));
    ...
    for (let i = 0; i <= gridLines; i++) {
      ...
      ctx.font = isMobile ? 'bold 9px monospace' : 'bold 13px monospace';
      ctx.textAlign = 'right';
      const decimals = zoom > 2 ? 2 : 1;
      ctx.fillText(`${value.toFixed(decimals)}%`, margin.left - (isMobile ? 5 : 15), y + 5);
    }
    ...
    // Draw legend on the right - only show on desktop
    if (!isMobile) {
    ...
    ctx.fillStyle = '#ffffff';
    ctx.font = isMobile ? 'bold 8px monospace' : 'bold 12px monospace';
    ctx.textAlign = 'center';

    const xLabelCount = isMobile ? 3 : Math.min(12, Math.max(6, Math.floor(6 * zoom)));
    for (let i = 0; i <= xLabelCount; i++) {
      ...
      if (performanceData[0]?.data[dataIndex]) {
        ...
        if (x >= margin.left && x <= margin.left + chartWidth) {
          ...
          ctx.fillText(label, x, dimensions.height - margin.bottom + (isMobile ? 20 : 30));
        }
      }
    }
    ...
    drawChart();
  }, [performanceData, dimensions, hoveredSector, visibleRange, activeCategories, isBenchmarked, crosshair, isMobile]);
  ...
  return (
    <div className="sector-performance-chart" style={{ 
      width: '100%',
      maxWidth: '1000px',
      height: isMobile ? '195px' : '600px',
      minHeight: isMobile ? '195px' : '600px',
      ...
      <div style={{ 
        ...
        gap: isMobile ? '2px' : '20px'
          ...
          <select
            ...
            style={{
              padding: isMobile ? '2px 3px' : '6px 12px',
        ...
        <label style={{
          ...
          <input
            type="checkbox"
            checked={activeCategories.growth}
            onChange={() => toggleCategory('growth')}
        ...
        <label style={{
          ...
          <input
            type="checkbox"
            checked={activeCategories.value}
        ...
        <label style={{
          ...
          <input
            type="checkbox"
            checked={activeCategories.defensives}
            onChange={() => toggleCategory('defensives')}
            style={{ cursor: 'pointer' }}
          />
          Defensives
        </label>
        ...
        <button
          onClick={() => {
            console.log('Benchmark clicked, current:', isBenchmarked);
            setIsBenchmarked(!isBenchmarked);
          }}
          style={{
            padding: isMobile ? '2px 4px' : '6px 12px',
            backgroundColor: isBenchmarked ? '#9d4edd' : '#1a1a1a',
            color: isBenchmarked ? '#ffffff' : '#ffffff',
            border: isBenchmarked ? '1px solid #9d4edd' : '1px solid #666666',
            borderRadius: '2px',
            cursor: 'pointer',
            fontSize: isMobile ? '8px' : '11px',
            ...
            <>
              <label
                style={{
                  padding: '2px 3px',
                  backgroundColor: '#1a1a1a',
                  color: customStartDate ? '#00ff00' : '#ffffff',
              ...
              <label
                style={{
                  padding: '2px 3px',
                  backgroundColor: '#1a1a1a',
                  color: customEndDate ? '#00ff00' : '#ffffff',
                  border: '1px solid #333333',
                  borderRadius: '2px',
                  fontSize: '8px',
                  fontFamily: 'monospace',
