    ...
    >
          ...
          <>
            ...
            >
              ...
              >
                ...
                <span style={{ color: '#00ff41', fontSize: '16.8px' }}>
      ...
      {currentTicker && (
        ...
        >
          ...
          `}</style>
          <EFIChart symbol={currentTicker} initialTimeframe="1d" height={600} />
        </div>
      )}
...
function IVChartsPanel({ data, ticker, period, onPeriodChange, isScanning }: IVChartsPanelProps) {
  ...
  const drawChart = (
    ...
    data.forEach((point, i) => {
      const x = padding.left + (chartWidth * i) / (data.length - 1)
      const y =
        padding.top + chartHeight - (((point[key] as number) - minValue) / range) * chartHeight
    ...
    const lastY =
      padding.top + chartHeight - ((values[values.length - 1] - minValue) / range) * chartHeight
  ...
  useEffect(() => {
    if (data.length === 0) return
    if (!callPutIVCanvasRef.current || !ivRankCanvasRef.current || !ivPercentileCanvasRef.current)
      return
