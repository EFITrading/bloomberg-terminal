...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const { symbols, dataTypes, timeframe = '1d', period = '1y', maxParallel = 6 } = body
    ...
    const results: TickerDataResponse[] = await Promise.all(
      symbols.map((symbol) =>
        fetchAllDataForSymbol(symbol, dataTypes, timeframe, dateRanges, maxParallel)
      )
    )
...
async function fetchOptionsData(symbol: string) {
  ...
  const optionsPromises = expirations.map((exp) =>
    fetch(
      `https://api.polygon.io/v3/snapshot/options/${symbol}?expiration_date=${exp}&limit=250&apikey=${POLYGON_API_KEY}`,
      {
