...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (cached && now - cached.timestamp < CACHE_TTL) {
      console.log(
        `⚡ DEALER CACHE HIT: ${ticker} (${Math.round((now - cached.timestamp) / 1000)}s old)`
      )
      return NextResponse.json(cached.data)
    }

    console.log(
      `🔥 DEALER CACHE MISS: Fetching fresh data for ${ticker}${specificExpiration ? ` expiration ${specificExpiration}` : ''}`
    )
