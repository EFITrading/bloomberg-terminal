...
export async function GET(request: NextRequest) {
  ...
  const ticker = searchParams.get('ticker') || 'VIX';
  const specificExpiration = searchParams.get('expiration');
  const apiKey = process.env.POLYGON_API_KEY;

  try {
    // Get current VIX price from underlying_asset (VIX index data not available on basic plan)
    let currentPrice: number | null = null;

    if (specificExpiration) {
      let allContracts: any[] = [];
      let nextUrl: string | null = `https://api.polygon.io/v3/snapshot/options/I:VIX?expiration_date=${specificExpiration}&limit=250&apikey=${apiKey}`;     
    ...
    let nextUrl: string | null = `https://api.polygon.io/v3/reference/options/contracts?underlying_ticker=VIX&limit=1000&apikey=${apiKey}`;
    ...
    for (const expDate of validExpirations) {
      let allContracts: any[] = [];
      let nextUrl: string | null = `https://api.polygon.io/v3/snapshot/options/I:VIX?expiration_date=${expDate}&limit=250&apikey=${apiKey}`;
