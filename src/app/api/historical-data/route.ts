...
export async function GET(request: NextRequest) {
  ...
  const keepDesc = searchParams.get('keepDesc') === 'true' // For industry analysis
  ...
  try {
    ...
    if (!response.ok) {
      if (response.status === 401) {
        ...
        const anyCachedData = Array.from(cache.values()).find(
          (cached) => cached.data && cached.data.results && cached.data.results.length > 0
        )
        if (anyCachedData) {
        ...
        throw new Error(
          'Access forbidden. Your API key may not have permission to access this data.'
        )
      } else {
