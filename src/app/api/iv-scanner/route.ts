...
async function getOptionsIV(
  ticker: string,
  weeks: number = 3
): Promise<{ callIV: number; putIV: number; price: number; expiration: string } | null> {
  try {
    ...
    // Find ATM options
    const sortedByExpiration = optionsData.results.sort(
      (a: any, b: any) =>
        new Date(a.expiration_date).getTime() - new Date(b.expiration_date).getTime()
    )

    const nearestExpiration = sortedByExpiration[0].expiration_date
    const expirationOptions = sortedByExpiration.filter(
      (opt: any) => opt.expiration_date === nearestExpiration
    )
...
export async function GET(request: NextRequest) {
  try {
    ...
    // Check if it's the right time (6:49 AM PST or 12:41 PM PST)
    if (!manualRun) {
    ...
    for (let i = 0; i < Math.min(TOP_1000_SYMBOLS.length, 100); i += batchSize) {
      ...
      console.log(
        `📊 Batch ${Math.floor(i / batchSize) + 1} complete: ${successCount} success, ${errorCount} errors`
      )
    }
