...
export async function GET() {
    try {
        ...
        const [barsResults, vixPrice, t10y3mObs, dffObs, hySpreadsObs, sentimentObs] = await Promise.all([
        ...
        // Fed policy
        if (fedCutting) macroBias -= 0.5   // cutting = policy easing = bull
        else if (fedFunds > 4.5) macroBias += 0.4   // high rates = late cycle pressure

        // Consumer sentiment
        if (sentiment > 85) macroBias -= 0.4   // euphoria = late bull
        else if (sentiment > 70) macroBias -= 0.2
        else if (sentiment < 55) macroBias += 0.4   // depression = bear/bottom
        ...
        const confidence = Math.round(Math.max(30, Math.min(95, 85 - spread * 8 + vixConfidence + macroConfidence)))
