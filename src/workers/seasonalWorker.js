...
async function processStockSymbols(symbols, years, spyData) {
  ...
  symbolResults.forEach((result, index) => {
    if (result.status === 'fulfilled' && result.value) {
      ...
      console.warn(` [Worker] Failed ${symbols[index]}:`, result.reason?.message)
    }
  })
...
async function processStockSymbol(symbol, years, spyData) {
  try {
    ...
    if (!service) {
      console.warn(` [Worker] No PolygonService available for ${symbol}`)
    ...
    const stockData = await service.getBulkHistoricalData(symbol, 30)
...
function calculateWeeklyReturns(data) {
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    if (weekDay === 1 || weekStart === null) {
      // Monday or first data point

      weekStart = date

      weekStartPrice = price
    } else if (weekDay === 5 || i === data.length - 1) {
      // Friday or last data point
...
function isSeasonalCurrentlyActive(startDate) {
  ...
  return (
    (daysDifference >= 1 && daysDifference <= 3) || (daysDifference >= -2 && daysDifference <= 0)
  )
}
