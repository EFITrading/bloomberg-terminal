...
export function useWatchlist() {
    ...
    const fetchWatchlistData = async () => {
        try {
            ...
            if (validResults.length === 0) {
                ...
                // Seed prevClose from REST data so WS can calculate daily % change
    ...
    useEffect(() => {
            ...
            ws.onmessage = (evt: MessageEvent) => {
                ...
                for (const msg of messages) {
                    if (msg.ev === 'status' && msg.status === 'auth_success') {
                        ws.send(JSON.stringify({ action: 'subscribe', params: symbols.map(s => `AM.${s}`).join(',') }));
                    }

                    if (msg.ev === 'AM' && symbols.includes(msg.sym)) {
        ...
        // Seed REST first so prevCloseRef is populated before auth_success fires
