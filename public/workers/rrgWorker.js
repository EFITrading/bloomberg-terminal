// Polygon API key injected at runtime by the page that spawns this worker
const POLYGON_API_KEY = self.__POLYGON_API_KEY__ || ''
...
async function calculateCustomRRG(symbols, benchmark, weeks, rsPeriod, momentumPeriod) {
  ...
  for (const symbol of symbols) {
    try {
      ...
      const relativeStrength = alignedSymbolPrices.map(
        (sp, i) => sp.close / alignedBenchmarkPrices[i].close
      )
      ...
      const rsMomentum =
        rsMomentumValues.length > 0 ? 100 + rsMomentumValues[rsMomentumValues.length - 1] : 100

      const priceChangePercent =
        symbolPrices.length >= 2
          ? ((symbolPrices[symbolPrices.length - 1].close -
              symbolPrices[symbolPrices.length - 2].close) /
              symbolPrices[symbolPrices.length - 2].close) *
            100
          : 0
