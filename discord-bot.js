...
async function generateFlowImage(data, efiOnly = false) {
  ...
  ctx.fillRect(
    ...
    0,
    baseWidth,
    baseHeaderHeight + baseColumnHeaderHeight + displayTrades.length * baseRowHeight + 20
  )
...
function formatTradesEmbed(data, title, efiOnly = false) {
  ...
  if (trades.length > 10) {
    embed.setDescription(
      `⚠️ Showing top 10 of **${trades.length}** trades. Use the web terminal for full details.`
    )
  }
  ...
  for (const trade of displayTrades) {
    ...
    const typeEmoji =
      trade.trade_type === 'SWEEP' ? '⚡' : trade.trade_type === 'BLOCK' ? '🧱' : '📊'
    ...
    if (efiOnly && trade.positioning) {
      ...
      const percentChange =
        entryPrice > 0 ? (((currentPrice - entryPrice) / entryPrice) * 100).toFixed(2) : '0.00'
  ...
  try {
    ...
    if ((command === 'weekly' && args[1]?.toLowerCase() === 'range') || command === 'range') {
      if (!ticker && command !== 'range') {
        await message.reply(
          `❌ Please provide a ticker. Example: \`${BOT_PREFIX}SPY weekly range\``
        )
        return
      }
    ...
    const response = await fetch(`${BASE_URL}/api/chat`, {
      ...
      body: JSON.stringify({
        messages: [{ role: 'user', content: fullCommand }],
      }),
    })
