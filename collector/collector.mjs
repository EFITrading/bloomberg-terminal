...
function startCollecting() {
  ...
  // 1-second flush: classify + enrich + push to allTrades
  flushTimer = setInterval(async () => {
    if (rawBuffer.length === 0) return
    const batch = rawBuffer.splice(0)
    console.log(`[FLUSH] ${batch.length} raw trades → enriching ...`)
    try {
      const enriched = await enrichBatch(batch)
      allTrades.push(...enriched)
      console.log(`[FLUSH] +${enriched.length} enriched | total: ${allTrades.length}`)
    } catch (err) {
      console.error('[FLUSH] Enrich error:', err.message)
      allTrades.push(...batch)  // push unenriched so data isn't lost
    }
  }, 1000)
